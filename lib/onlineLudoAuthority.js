...
function createTokens(players) {
  ...
  for (const player of players) {
    ...
    for (const color of colors) {
      for (let id = 0; id < TOKEN_COUNT; id++) {
        tokens[pid][`${color}:${id}`] = { position: 0 };
      }
    }
  }
  return tokens;
...
function colorsFor(state, playerId) {
  ...
  if (!player) return [];
  return player.colors?.length
    ? player.colors
    : playerColorsForSeats(state.players.length === 2 ? 2 : 4, player.seat);
}
...
function move(state, playerId, tokenId) {
  ...
  const id = Number(idText);
  if (!player || !colors.includes(color) || !Number.isInteger(id)) {
    return { ok: false, reason: "not_your_token" };
  }

  const token = all.find(
    (t) => String(t.playerId) === String(playerId) && t.color === color && t.id === id
  );
  ...
  const moved = all.map((t) =>
    t.playerId === token.playerId && t.color === token.color && t.id === token.id
      ? { ...t, position: target, state: tokenState(target) }
      : t
  );

  const movedToken = moved.find(
    (t) => t.playerId === token.playerId && t.color === token.color && t.id === token.id
  );

  let captured = null;
  if (movedToken) {
    const cell = require("./ludoRules").getTokenCell(color, target);
    if (target > 0 && target < 52 && cell) {
      const opponents = moved.filter(
        (t) =>
          t.position > 0 &&
          t.position < FINISH &&
          !colors.includes(t.color) &&
          JSON.stringify(require("./ludoRules").getTokenCell(t.color, t.position)) === JSON.stringify(cell)
      );
      if (opponents.length === 1) {
        captured = opponents[0];
        const index = moved.indexOf(captured);
        moved[index] = { ...captured, position: 0, state: "yard" };
      }
    }
  }
