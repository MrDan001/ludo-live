...
function registerRoomStake(code, hostPlayerId, stakeCoins) {
  const normalizedCode = normalizeCode(code);
  const normalizedStake = normalizeStake(stakeCoins) || 0;
  const hostId = String(hostPlayerId || '').trim();
  if (!normalizedCode || !hostId) throw new Error('Room code and host are required.');
  const existing = roomStakes.get(normalizedCode);
  if (existing && existing.hostPlayerId !== hostId) throw new Error('This room already exists.');
  roomStakes.set(normalizedCode, {
    hostPlayerId: hostId,
    stakeCoins: normalizedStake,
    createdAt: existing?.createdAt || Date.now(),
    updatedAt: Date.now(),
  });
  return { stakeType: normalizedStake > 0 ? 'paid' : 'free', stakeCoins: normalizedStake, paid: normalizedStake > 0 };
}
function getRoomStake(code) {
  const entry = roomStakes.get(normalizeCode(code));
  if (!entry) return { stakeType: 'free', stakeCoins: 0, paid: false };
  const stakeCoins = Number(entry.stakeCoins) || 0;
  return { stakeType: stakeCoins > 0 ? 'paid' : 'free', stakeCoins, paid: stakeCoins > 0, hostPlayerId: entry.hostPlayerId };
}
function deleteRoomStake(code) { roomStakes.delete(normalizeCode(code)); }
...
async function roomStake(code) {
  ...
  return {
    betId: String(row.id), roomCode: normalized, roomSize: Number(row.mode_players) || 2,
    stakePerPlayer: Number(row.stake_per_player) || 0, pot: Number(row.pot) || 0,
    stakedAmount: Number(row.staked_amount) || 0, stakedPlayers: Number(row.staked_players) || 0,
    status: String(row.status || 'open'), winnerId: row.winner_id ? String(row.winner_id) : null,
    createdAt: row.created_at, startedAt: row.started_at, settledAt: row.settled_at,
  };
}

async function ensurePlayerStake({ code, playerId, roomSize, stake }) {
  const normalizedCode = normalizeCode(code);
  ...
  try {
    ...
    if (bet && ['settled','refunded','cancelled'].includes(String(bet.status))) throw new Error('This room is no longer accepting stakes.');
    if (bet && Number(bet.stake_per_player) !== normalizedStake) throw new Error('The room stake has changed. Please rejoin using the current room stake.');
    if (bet && Number(bet.mode_players) !== size) throw new Error('The room player count does not match the stake configuration.');
    if (!bet) {
      const created = await client.query(`INSERT INTO ludo_multiplayer_match_bets(room_code,mode_players,stake_per_player,pot,status)
        VALUES($1,$2,$3,0,'open') ON CONFLICT (room_code) DO UPDATE SET room_code=EXCLUDED.room_code RETURNING *`, [normalizedCode, size, normalizedStake]);
      bet = created.rows[0];
    }
    ...
    if (!existingPlayer.rowCount) {
      ...
      await adjustCoins(client, pid, -normalizedStake, { source:'multiplayer_stake', sourceRef, actorUserId:pid, actorType:'user', reason:`Multiplayer stake of ${normalizedStake.toLocaleString()} coins for room ${normalizedCode}` });
...
async function releasePlayerStake(code, playerId) {
  const normalizedCode = normalizeCode(code); const pid = String(playerId || '').trim(); if (!normalizedCode || !pid) return;
  await ensureSchema(); const client = await db().connect();
  try {
    await client.query('BEGIN');
    ...
    await adjustCoins(client, pid, stake, { source:'multiplayer_stake_refund', sourceRef:`${normalizedCode}:bet:${bet.rows[0].id}:refund:${pid}`, actorType:'system', reason:`Refund of ${stake.toLocaleString()} coins because room ${normalizedCode} was left before the match started` });
    ...
    await client.query('COMMIT');
  } catch (error) { try { await client.query('ROLLBACK'); } catch {} console.error('[multiplayer-stake] refund failed', normalizedCode, pid, error); }
  finally { client.release(); }
}

async function markMatchStarted(code) {
  const normalizedCode = normalizeCode(code); if (!normalizedCode) return;
  await ensureSchema();
  try { await db().query(`UPDATE ludo_multiplayer_match_bets SET status='locked',started_at=COALESCE(started_at,NOW()) WHERE room_code=$1 AND status='open'`, [normalizedCode]); }
...
async function settleMatch(code, winnerId) {
  const normalizedCode = normalizeCode(code); const winner = String(winnerId || '').trim(); if (!normalizedCode) return;
  await ensureSchema(); const client = await db().connect();
  try {
    await client.query('BEGIN');
    ...
    const bet = betResult.rows[0]; if (!['open','locked'].includes(String(bet.status))) { await client.query('COMMIT'); return; }
    ...
    if (winner) {
      if (!players.rows.some(p => String(p.user_id) === winner)) throw new Error('Winner did not fund this multiplayer match.');
      if (pot > 0) await adjustCoins(client, winner, pot, { source:'multiplayer_stake_payout', sourceRef:`${normalizedCode}:bet:${bet.id}:payout`, actorType:'system', reason:`Winner payout of ${pot.toLocaleString()} coins for multiplayer room ${normalizedCode}` });
      ...
      for (const player of players.rows) {
        const stake = Number(player.stake) || 0; if (!stake) continue;
        await adjustCoins(client, String(player.user_id), stake, { source:'multiplayer_stake_refund', sourceRef:`${normalizedCode}:bet:${bet.id}:refund:${String(player.user_id)}`, actorType:'system', reason:`Refund of ${stake.toLocaleString()} coins because multiplayer room ${normalizedCode} ended without a winner` });
      }
      await client.query("UPDATE ludo_multiplayer_match_bets SET status='refunded',settled_at=NOW() WHERE id=$1", [bet.id]);
    }
    await client.query('COMMIT');
  } catch (error) { try { await client.query('ROLLBACK'); } catch {} console.error('[multiplayer-stake] settlement failed', normalizedCode, error); }
  finally { client.release(); }
}

module.exports = { MIN_STAKE, MAX_STAKE, normalizeCode, normalizeStake, registerRoomStake, getRoomStake, deleteRoomStake, ensureSchema, roomStake, ensurePlayerStake, releasePlayerStake, markMatchStarted, settleMatch };
