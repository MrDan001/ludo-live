...
function opponentsAt(tokens,color,progress){const cell=getTokenCell(color,progress);if(!cell)return [];return tokens.filter(t=>t.position>0&&t.position<FINISH&&t.color!==color&&same(getTokenCell(t.color,t.position),cell))}
function canMove(tokens,token,dice){if(token.position>=FINISH||token.state==="finished")return false;const target=nextProgress(token.position,dice);if(target===null)return false;if(target===FINISH)return true;return opponentsAt(tokens,token.color,target).length<2}
...
function applyMove(tokens,token,dice){if(!canMove(tokens,token,dice))return null;const target=nextProgress(token.position,dice);if(target===null)return null;let moved=tokens.map(item=>item.color===token.color&&item.id===token.id?{...item,position:target,state:tokenState(target)}:item);let captured=null;if(target!==FINISH&&!isSafeProgress(token.color,target)){const opponents=opponentsAt(moved,token.color,target);if(opponents.length===1){captured=opponents[0];moved=moved.map(item=>item.color===captured.color&&item.id===captured.id?{...item,position:0,state:"yard"}:item)}}return {tokens:moved,target,captured};}
