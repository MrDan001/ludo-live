...
function getTokenCell(color,progress){if(progress>=1&&progress<=TRACK_LENGTH)return getTrackCell(color,progress);if(progress>=HOME_START&&progress<FINISH)return getHomeCell(color,progress);if(progress===FINISH)return FINISH_CELL;return null}
...
module.exports={COLORS,START_INDEX,TRACK_LENGTH,PHYSICAL_TRACK_LENGTH,HOME_START,HOME_STRETCH,FINISH,FINISH_CELL,MAIN_PATH,HOME_LANES,SAFE_CELLS,getTrackCell,getHomeCell,getTokenCell,tokenState,nextProgress,isSafeProgress,opponentsAt,canMove,legalMoves,hasLegalMove,applyMove,hasWon,winner,playerColorsForSeats};
