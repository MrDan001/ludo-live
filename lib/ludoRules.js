...
// These rules intentionally mirror the restored Bot-vs-Human board. The bot board
// is the gameplay reference: entry, path/blockade validation, captures, home path,
// safe squares and finish handling all follow its proven movement set.
function canMove(tokens,token,dice){
  ...
  for(let step=token.position+1;step<=Math.min(target,HOME_START);step++){
    ...
    const occupants=tokens.filter(t=>t.position>0&&t.position<FINISH&&t.position===step&&same(getTokenCell(t.color,t.position),cell));
    if(occupants.filter(t=>t.color!==token.color).length>=2)return false;
  }
  ...
  return occupants.filter(t=>t.color!==token.color).length<2;
}
...
function applyMove(tokens,token,dice){
  ...
  let moved=tokens.map(item=>item.color===token.color&&item.id===token.id?{...item,position:target,state:tokenState(target)}:item);
  let captured=null;
