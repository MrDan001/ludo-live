...
function applyMove(tokens,token,dice){
 ...
 if(target<HOME_START&&!isSafeProgress(token.color,target)){const opponents=opponentsAt(moved,token.color,target);if(opponents.length===1){captured=opponents[0];moved=moved.map(item=>item.color===captured.color&&item.id===captured.id?{...item,position:0,state:"yard"}:item)}}
