...
function RoomContent(){
 ...
 const start=()=>{window.location.href=`/game-online?room=${encodeURIComponent(room!.code)}&name=${encodeURIComponent(room!.name)}${room!.host?"&host=1":""}`};
 if(room)return <AppFrame back="/lobby"><div style={{maxWidth:900,margin:"0 auto"}}>
  ...
  <div style={{marginTop:18,padding:18,borderRadius:18,background:"linear-gradient(135deg,#071c45,#0b1630)",border:"1px solid rgba(59,130,246,.22)"}}><div style={{fontWeight:900,fontSize:18}}>🎙️ Live room</div><p style={{color:"#94a3b8",marginBottom:0}}>Players waiting can discover this room in Online Players. Voice chat, quick messages, readiness and host moderation are live.</p></div>
  <LiveSocial roomCode={room.code} name={room.name} host={room.host} roomSize={room.players} onStart={start} onKicked={()=>{localStorage.removeItem("ludo-room");window.location.href="/lobby"}}/>
