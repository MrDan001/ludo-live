...
type DiceItem=[string,string,boolean];
const dice:DiceItem[]=[["⚪","Classic Dice",true],["🎲","Fire Dice",false],["🔵","Ice Dice",false],["🟡","Gold Dice",false],["🟣","Neon Dice",false],["🟠","Cube Dice",false],["🔷","Crystal Dice",false],["⚫","Skull Dice",false],["🔮","Galaxy Dice",false],["💗","Heart Dice",false],["⚙️","Robot Dice",false],["🟢","Lucky Dice",false]];
export default function InventoryPage(){const [tab,setTab]=useState("Dice");const [wallet,setWallet]=useState<Wallet>({coins:25680,gems:320,spins:0,mystery:0});useEffect(()=>{const r=()=>{try{const stored=JSON.parse(localStorage.getItem("ludo-wallet")||"{}");setWallet(prev=>({...prev,...stored}))}catch{}};r();window.addEventListener("ludo-wallet-updated",r);window.addEventListener("storage",r);return()=>{window.removeEventListener("ludo-wallet-updated",r);window.removeEventListener("storage",r)}},[]);return <AppFrame back="/home"><main style={page}><header style={header}><div><div style={eyebrow}>PLAYER INVENTORY</div><h1 style={title}>Inventory</h1></div><div style={walletBadge}>{wallet.coins.toLocaleString()} 🪙<br/><span>{wallet.gems.toLocaleString()} 💎</span></div></header><div style={tabs}>{["Dice","Boards","Avatar","Friends"].map(t=><button key={t} onClick={()=>setTab(t)} style={{...tabStyle,...(tab===t?tabActive:{})}}>{t}</button>)}</div>{tab==="Dice"?<><section style={grid}>{dice.map(([icon,name,owned])=><article key={name} style={{...item,opacity:owned?1:.82}}><div style={{fontSize:42,filter:owned?"none":"grayscale(.45)"}}>{icon}</div><b>{name}</b>{owned?<span style={ownedTag}>✓ Owned</span>:<span style={locked}>🔒</span>}</article>)}</section><div style={hint}>Collect more dice from events, missions and the shop.</div></>:<section style={empty}><div style={{fontSize:42}}>✨</div><h2>{tab}</h2><p>Customize this part of your player inventory as you unlock more items.</p></section>}</main></AppFrame>}
...
const walletBadge={textAlign:"right" as const,fontSize:12,color:"#f5c84b",fontWeight:900};
...
const tabStyle={border:0,borderRadius:8,padding:"10px 3px",color:"#fff",fontSize:11,fontWeight:900,cursor:"pointer"};
