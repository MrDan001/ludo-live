...
function socket(){ return io(SOCKET_URL, { transports: ["websocket", "polling"] }); }
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const createRoom = () => {
    ...
    window.location.href = `/room?code=${code}&size=${roomSize}&host=1`;
  };

  const joinRoom = (code: string, size: number) => {
    localStorage.setItem("ludo-player-name", name.trim() || "PlayerOne");
    window.location.href = `/room?code=${code}&size=${size}`;
  };
    ...
    <main className="app-home">
      <header className="home-topbar">
        <div className="player-mini">
          ...
          <div><b>{name || "PlayerOne"}</b><span>Level 1 · Online</span></div>
        </div>
      ...
      <section className="hero-card">
        <div className="hero-copy"><small>LU DO LIVE</small><h1>PLAY. CONNECT. WIN.</h1><p>Real rooms. Real players. One live game.</p></div>
      ...
      <section className="quick-row">
        <a href="/missions">🎁<b>Daily Reward</b></a>
        <a href="/shop">🛒<b>Shop</b></a>
        <a href="/events">📅<b>Events</b></a>
        <a href="/spin">🎡<b>Spin Wheel</b></a>
      </section>

      <section className="online-panel">
        <div className="section-head"><div><b>🟢 PLAYERS WAITING</b><span>{rooms.length ? `${rooms.length} open rooms` : "No open rooms yet"}</span></div><a href="/rooms">VIEW ALL ›</a></div>
        {rooms.slice(0, 3).map(room => <div className="room-line" key={room.code}><div><b>{room.hostName}'s room</b><span>Room {room.code} · {room.players}/{room.roomSize}</span></div><button onClick={() => joinRoom(room.code, room.roomSize)}>JOIN</button></div>)}
        {!rooms.length && <div className="empty-state">Create the first room and other online players will see it here.</div>}
      </section>

      <section className="create-card">
        <div><b>CREATE YOUR OWN ROOM</b><span>Choose 2 or 4 players. Your room appears live to everyone.</span></div>
        <button onClick={() => setShowCreate(true)}>CREATE ROOM</button>
      </section>

      <nav className="bottom-nav">
        <a className="active" href="/">⌂<span>Home</span></a><a href="/rooms">👥<span>Players</span></a><a href="/chat">💬<span>Chat</span></a><a href="/profile">●<span>Profile</span></a>
      </nav>
