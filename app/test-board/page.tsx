...
function Die({ value, used, active, onClick, disabled }: { value: number; used: boolean; active: boolean; onClick: () => void; disabled: boolean }) {
  return <button type="button" onClick={onClick} disabled={disabled} aria-label={`Die ${value}`} className={["relative h-16 w-16 rounded-2xl bg-white text-slate-900 shadow-xl transition-all", active ? "scale-110 ring-4 ring-yellow-300" : "", used ? "opacity-30" : "", disabled ? "cursor-not-allowed" : "active:scale-95"].join(" ")}><span className="text-4xl leading-none" aria-hidden="true">🎲</span><span className="absolute inset-0 flex items-center justify-center text-lg font-black">{used ? "✓" : value || ""}</span></button>;
}
export default function TestBoardPage() {
  ...
  const finishOrContinue = (nextState: GameState, nextUsed: UsedDice, roll: DiceRoll) => {
    if (nextState.winner) { setGameState(nextState); setDiceRoll(null); setValidMoves([]); setActiveSource(null); return; }
