...
const START_INDEX: Record<Color, number> = { green: 15, yellow: 27, blue: 40, red: 1 };
...
export default function HomePage() {
  ...
  function moveToken(tokenIndex: number) {
    ...
    if (current < 0) {
      ...
      setTokens({ ...tokens, [turn]: tokens[turn].map((p,i) => i === tokenIndex ? 0 : p) });
      setMessage(`${turn.toUpperCase()} token ${tokenIndex + 1} entered on its colored first-exit square.`); finishTurn(); return;
    }
  ...
  return <main className="board-page"><section className="game-shell">
    ...
    <div className="ludo-board" aria-label="Ludo board">
      ...
      <Home color="green" className="home-green" tokens={tokens.green} onTokenClick={i=>moveToken(i)}/><Home color="yellow" className="home-yellow" tokens={tokens.yellow} onTokenClick={i=>moveToken(i)}/><Home color="red" className="home-red" tokens={tokens.red} onTokenClick={i=>moveToken(i)}/><Home color="blue" className="home-blue" tokens={tokens.blue} onTokenClick={i=>moveToken(i)}/><Center/>
    </div>
  </section></main>;
}
