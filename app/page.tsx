...
// Shared-track exits: red and green remain as before; yellow exits from the
// right arm and blue from the left arm. After one full shared-track circuit,
// each colour enters its own five-square final home lane toward the centre.
const START_INDEX: Record<PlayerColor, number> = { red: 0, green: 17, yellow: 33, blue: 9 };
...
export default function HomePage() {
  ...
  const playersRef = useRef(players); const turnRef = useRef(turn); const doubleSixesRef = useRef(doubleSixes);
  playersRef.current = players; turnRef.current = turn; doubleSixesRef.current = doubleSixes;
  ...
  async function moveHuman(id: number) { if (!choice || turn !== "red" || moving || forfeited) return; const selectedChoice = choice, value = selectedChoice === "blue" ? dice[0] : selectedChoice === "green" ? dice[1] : total; if (value === null) return; const token = playersRef.current.find(p => p.color === "red")?.tokens.find(t => t.id === id); if (!token || !legal(token, value, selectedChoice === "red")) return; setMoving(true); try { if (token.status === "home") { setPlayers(s => s.map(p => p.color === "red" ? { ...p, tokens: p.tokens.map(t => t.id === id ? { ...t, status: "track", progress: 0 } : t) } : p)); await new Promise(r => window.setTimeout(r, 220)); } else { for (let n = 1; n <= value; n++) { setPlayers(s => s.map(p => p.color === "red" ? { ...p, tokens: p.tokens.map(t => t.id === id ? { ...t, status: t.progress + 1 === FINISH_PROGRESS ? "finished" : "track", progress: t.progress + 1 } : t) } : p)); await new Promise(r => window.setTimeout(r, 170)); } } const after = playersRef.current.find(p => p.color === "red")?.tokens.find(t => t.id === id); if (after) setPlayers(s => killOneOpponent(s, "red", after)); const nextUsed: [boolean, boolean] = [used[0] || selectedChoice === "blue" || selectedChoice === "red", used[1] || selectedChoice === "green" || selectedChoice === "red"]; setUsed(nextUsed); setChoice(null); if (nextUsed[0] && nextUsed[1]) { const extra = dice[0] === 6 || dice[1] === 6; clearDice(); if (extra) setDoubleSixes(0); else nextTurn("red"); } } finally { setMoving(false); } }
  useEffect(() => { if (turn === "red" || botBusy) return; setBotBusy(true); setChoice(null); let cancelled = false; const botColor = turn; const timer = window.setTimeout(async () => { try { if (cancelled) return; const a = Math.floor(Math.random() * 6) + 1, b = Math.floor(Math.random() * 6) + 1, isDoubleSix = a === 6 && b === 6, nextDoubleCount = isDoubleSix ? doubleSixesRef.current + 1 : 0; setDice([a, b]); setUsed([false, false]); setDoubleSixes(nextDoubleCount); await new Promise(r => window.setTimeout(r, 550)); if (cancelled) return; if (nextDoubleCount >= 3) { setMoving(false); setBotBusy(false); nextTurn(botColor); return; } setMoving(true); for (const value of [a, b] as const) { if (cancelled) return; const state = playersRef.current, id = chooseBotToken(state, botColor, value), token = id === null ? null : state.find(p => p.color === botColor)?.tokens.find(t => t.id === id); if (!token || !isMovable(token, value)) continue; if (token.status === "home") { setPlayers(s => applyMove(s, botColor, id, value)); await new Promise(r => window.setTimeout(r, 220)); } else { for (let n = 1; n <= value; n++) { if (cancelled) return; setPlayers(s => s.map(p => p.color === botColor ? { ...p, tokens: p.tokens.map(t => t.id === id ? { ...t, progress: t.progress + 1, status: t.progress + 1 === FINISH_PROGRESS ? "finished" : "track" } : t) } : p)); await new Promise(r => window.setTimeout(r, 170)); } } const after = playersRef.current.find(p => p.color === botColor)?.tokens.find(t => t.id === id); if (after) setPlayers(s => killOneOpponent(s, botColor, after)); } if (!cancelled) { setMoving(false); setBotBusy(false); setDice([null, null]); setUsed([false, false]); if (a === 6 || b === 6) { setDoubleSixes(0); setBotCycle(v => v + 1); } else { setDoubleSixes(0); setTurn(NEXT[botColor]); } } } catch { if (!cancelled) { setMoving(false); setBotBusy(false); clearDice(); nextTurn(botColor); } } }, 500); return () => { cancelled = true; window.clearTimeout(timer); }; }, [turn, botCycle]);
  const chosen = choice === "blue" ? dice[0] : choice === "green" ? dice[1] : total; const canMove = chosen !== null && me.tokens.some(t => legal(t, chosen, choice === "red")); const homeSix = (choice === "blue" || choice === "green") && chosen === 6; const playerByColor = (c: PlayerColor) => players.find(p => p.color === c)?.tokens ?? [];
  return <main className="game-page"><div className="game-stage"><div className="board-wrap"><div className="ludo-board"><div className="track">{Array.from({ length: 15 }, (_, row) => Array.from({ length: 15 }, (_, col) => ((row >= 6 && row <= 8) || (col >= 6 && col <= 8)) ? <TrackCell key={`${row}-${col}`} row={row} col={col} /> : <div key={`${row}-${col}`} className="empty-cell" />))}</div><Home color="green" name="Player1" tokens={playerByColor("green")} /><Home color="yellow" name="Player2" tokens={playerByColor("yellow")} /><Home color="blue" name="Player4" tokens={playerByColor("blue")} /><Home color="red" name="Me" tokens={me.tokens}><div className="tokens">{me.tokens.filter(t => t.status === "home").map(t => <button key={t.id} className={`token-slot token-home-button ${homeSix ? "token-movable-home" : ""}`} onClick={() => moveHuman(t.id)} disabled={!homeSix}><div className="token" style={{ background: COLORS.red }} /></button>)}</div></Home><div className="board-token-layer">{players.flatMap(p => p.tokens.map(t => { const pnt = tokenPos(p.color, t.progress); if (!pnt) return null; const movable = p.color === "red" && canMove && legal(t, chosen!, choice === "red"); return <button key={`${p.color}-${t.id}`} className={`board-token ${movable ? "token-movable" : ""}`} style={{ ...pnt, background: COLORS[p.color] }} onClick={() => moveHuman(t.id)} disabled={!movable} />; }))}</div><div className="center-home"><div className="center-backdrop">LUDO</div><div className="center-controls"><div className="dice-pair"><Die value={dice[0]} onClick={roll} disabled={rolling || botBusy} /><Die value={dice[1]} onClick={roll} disabled={rolling || botBusy} /></div></div></div></div></div><div className="move-controls"><button className={`choice-token choice-blue ${choice === "blue" ? "chosen" : ""}`} disabled={turn !== "red" || !available[0] || !me.tokens.some(t => dice[0] !== null && legal(t, dice[0], false))} onClick={() => selectChoice("blue")}>{dice[0] ?? "?"}</button><button className={`choice-token choice-green ${choice === "green" ? "chosen" : ""}`} disabled={turn !== "red" || !available[1] || !me.tokens.some(t => dice[1] !== null && legal(t, dice[1], false))} onClick={() => selectChoice("green")}>{dice[1] ?? "?"}</button><button className={`choice-token choice-red ${choice === "red" ? "chosen" : ""}`} disabled={turn !== "red" || !available[0] || !available[1] || !me.tokens.some(t => total !== null && legal(t, total, true))} onClick={() => selectChoice("red")}>{total ?? "?"}</button></div></div></main>;
}

function TrackCell({ row, col }: { row: number; col: number }) { return <div className="track-cell" data-row={row} data-col={col} />; }
