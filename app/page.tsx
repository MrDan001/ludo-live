...
function RollToken({ color, value, selected, disabled, label, onClick }: { color: "blue" | "green" | "red"; value: number | null; selected: boolean; disabled?: boolean; label: string; onClick: () => void }) {
  return <button className={`roll-token roll-${color} ${selected ? "chosen" : ""}`} onClick={onClick} disabled={disabled} aria-label={label}>{value ?? "?"}</button>;
}
...
export default function HomePage() {
  ...
  return <main className="game-page"><div className="game-stage">
    <div className="dice-area" aria-label="Dice choices">
      <div className={`roll-tokens ${rolling ? "rolling" : ""}`}>
        <RollToken color="blue" value={dice[0]} selected={choice === "blue"} disabled={!canChoose} label="Play blue die" onClick={() => setChoice("blue")} />
        <RollToken color="green" value={dice[1]} selected={choice === "green"} disabled={!canChoose} label="Play green die" onClick={() => setChoice("green")} />
        <RollToken color="red" value={total} selected={choice === "red"} disabled={!canChoose} label="Merge both dice" onClick={() => setChoice("red")} />
      </div>
      <button className="roll-button" onClick={rollDice} disabled={rolling || forfeits}>{rolling ? "Rolling…" : "ROLL"}</button>
    ...
    <div className="board-wrap"><div className="ludo-board" aria-label="Ludo board">
      ...
      <div className="center-home" aria-label="Ludo center"><div className="triangle triangle-green" /><div className="triangle triangle-yellow" /><div className="triangle triangle-red" /><div className="triangle triangle-blue" /><div className="ludo-badge">LUDO</div></div>
    </div></div>
  </div></main>;
}
