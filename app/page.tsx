...
export default function HomePage(){
 ...
 return <main className="game-page"><div className="game-stage"><div className="board-wrap"><div className="ludo-board">
   <div className="track">{Array.from({length:225},(_,i)=>{const r=Math.floor(i/15),c=i%15,key=`${r}-${c}`;const isRoute=routeKey.has(key);const lane=laneKeys.get(key);return <div key={key} className={isRoute?`track-cell ${lane?`${lane}-path`:""} ${[0,11,17,35].some(n=>{const [rr,cc]=BOARD_ROUTE[n];return rr===r&&cc===c})?"start-cell":""}`:"empty-cell"}>{isRoute&&[0,11,17,35].some(n=>{const [rr,cc]=BOARD_ROUTE[n];return rr===r&&cc===c})?"★":""}</div>})}</div>
   ...
   <div className="center-home"><div className="center-backdrop">LUDO</div></div>
 ...
 <div className="move-controls"><div className="dice-pair"><Die value={dice[0]} onClick={()=>selectChoice("blue")} disabled={turn!=="red"||!available[0]||moving||rolling}/><Die value={dice[1]} onClick={()=>selectChoice("green")} disabled={turn!=="red"||!available[1]||moving||rolling}/></div><button className="choice-token choice-red" onClick={roll} disabled={turn!=="red"||rolling||moving||botBusy||forfeited}>ROLL</button></div>
 </div></main>
}
