...
const END_AT=Date.now()+((2*24+14)*60+30)*60*1000;
...
export default function TournamentPage(){const [wallet,setWallet]=useState(defaultWallet),[joined,setJoined]=useState(false),[notice,setNotice]=useState(""),[remaining,setRemaining]=useState(END_AT-Date.now());useEffect(()=>{setWallet(readWallet());const refresh=()=>setWallet(readWallet());window.addEventListener("ludo-wallet-updated",refresh);const t=window.setInterval(()=>setRemaining(END_AT-Date.now()),1000);return()=>{window.removeEventListener("ludo-wallet-updated",refresh);window.clearInterval(t)}},[]);const join=()=>{if(joined)return;if(wallet.coins<1000){setNotice("You need 1,000 coins to enter this tournament.");return}const w={...wallet,coins:wallet.coins-1000};localStorage.setItem("ludo-wallet",JSON.stringify(w));window.dispatchEvent(new Event("ludo-wallet-updated"));setWallet(w);setJoined(true);setNotice("You joined the Grand Tournament!")};return <AppFrame back="/home"><main style={page}><header style={header}><button onClick={()=>history.back()} style={back}>←</button><b>Tournament</b><button style={info}>ⓘ</button></header><section style={hero}><div style={heroArt}><span>🎲</span><span>🏆</span><span>🪙</span><span>💎</span></div><div style={eyebrow}>GRAND TOURNAMENT</div><div style={cup}>🏆</div><div style={endBadge}>Ends in: {formatRemaining(remaining)}</div></section><section style={card}><div style={prizeRow}><div><small>Prize Pool</small><strong>🪙 100,000</strong></div><div><small>&nbsp;</small><strong>💎 2,000</strong></div></div><div style={infoGrid}><div><small>Entry Fee</small><strong>🪙 1,000</strong></div><div><small>Players</small><strong>256 / 512</strong></div><div><small>Type</small><strong>Solo</strong></div></div><div style={rules}><div style={rulesTitle}>Rules</div><div>• 4 Players</div><div>• Standard Rules</div><div>• Top 3 Win Prizes</div><div style={rulesCup}>🏆</div></div><button onClick={join} disabled={joined||remaining<=0} style={{...joinBtn,opacity:joined||remaining<=0?.65:1}}>{joined?"Joined Tournament":remaining<=0?"Tournament Ended":"Join Tournament"}</button></section><button style={myBtn}>My Tournaments <span>›</span></button>{notice&&<div style={noticeStyle}>{notice}</div>}</main></AppFrame>}
