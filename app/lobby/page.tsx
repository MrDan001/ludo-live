...
export default function LobbyPage(){
 ...
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:22}}><button onClick={()=>openRoom("/free-play")} style={freeTile}><span style={{fontSize:30}}>🎮</span><b style={{display:"block",fontSize:20,marginTop:8}}>Free Play</b><small>No coins or stake locking. Play multiplayer Ludo for free.</small></button><button onClick={()=>openRoom("/room?action=create")} style={tile}><span style={{fontSize:30}}>➕</span><b style={{display:"block",fontSize:20,marginTop:8}}>Create Game</b><small>Open your own room and wait for players.</small></button><button onClick={()=>openRoom("/room?action=join")} style={tile}><span style={{fontSize:30}}>🔑</span><b style={{display:"block",fontSize:20,marginTop:8}}>Join by Code</b><small>Enter a friend's private room code.</small></button></div>
  ...
  <div style={flow}><b>Live matchmaking</b><br/><span>Free Play → Create/join a no-stakes room → Ready → Host starts → Play. Paid-stake rooms remain available separately.</span></div>
...
const freeTile={...tile,background:"linear-gradient(135deg,rgba(15,59,42,.98),rgba(7,27,43,.98))",border:"1px solid rgba(74,222,128,.35)"};
