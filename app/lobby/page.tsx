...
export default function LobbyPage(){
 ...
 const gatedHref=(href:string)=><a href={href} onClick={e=>{e.preventDefault();gate.check(()=>{window.location.href=href})}} style={tile}>;
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:22}}>{gatedHref("/room?action=create")}<span style={{fontSize:30}}>➕</span><b style={{display:"block",fontSize:20,marginTop:8}}>Create Game</b><small>Open your own room and wait for players.</small></a>{gatedHref("/room?action=join")}<span style={{fontSize:30}}>🔑</span><b style={{display:"block",fontSize:20,marginTop:8}}>Join by Code</b><small>Enter a friend's private room code.</small></a></div>
  <section style={{marginTop:28}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{margin:0}}>🔥 Rooms waiting for players</h2><span style={{color:"#64748b",fontSize:12}}>{rooms.length} open</span></div>
   {rooms.length===0?<div style={empty}>No open rooms right now.<br/><span>Create a game and your room will appear here automatically.</span></div>:<div style={{display:"grid",gap:10}}>{rooms.map(room=><div key={room.code} style={roomCard}><div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}><div style={{width:46,height:46,borderRadius:"50%",display:"grid",placeItems:"center",background:"linear-gradient(135deg,#1d4ed8,#22c55e)",fontSize:23}}>🎲</div><div><b style={{fontSize:16}}>{room.hostName}'s Game</b><div style={{color:"#94a3b8",fontSize:12,marginTop:3}}>Room {room.code} · {room.roomSize} players</div></div></div><div style={{display:"flex",alignItems:"center",gap:12}}><span style={{color:"#4ade80",fontWeight:900,fontSize:13}}>{room.players}/{room.roomSize}</span><a href={`/room?action=join&code=${room.code}&size=${room.roomSize}`} onClick={e=>{e.preventDefault();gate.check(()=>{window.location.href=`/room?action=join&code=${room.code}&size=${room.roomSize}`})}} style={joinBtn}>JOIN</a></div></div>)}</div>}
  </section>
...
const tile={textDecoration:"none",color:"#fff",padding:22,borderRadius:18,background:"linear-gradient(135deg,rgba(10,31,70,.95),rgba(8,20,43,.95))",border:"1px solid rgba(96,165,250,.2)"};
...
const joinBtn={textDecoration:"none",background:"#22c55e",color:"#04130a",padding:"10px 14px",borderRadius:10,fontWeight:950,fontSize:12};
