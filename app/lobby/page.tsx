...
export default function LobbyPage(){
 ...
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:22}}>
   <button onClick={()=>openRoom("/room?action=create")} style={tile}><span style={{fontSize:30}}>➕</span><b style={{display:"block",fontSize:20,marginTop:8}}>Create Game</b><small>Open your own room and play for coins.</small></button>
   ...
   <button onClick={()=>openRoom("/free-play")} style={freeTile}><span style={{fontSize:30}}>🎮</span><b style={{display:"block",fontSize:20,marginTop:8}}>Free Play</b><small>No coins. No stakes. Same live multiplayer Ludo.</small></button>
  </div>
...
const freeTile={...tile,background:"linear-gradient(135deg,rgba(6,78,59,.98),rgba(7,35,38,.98))",border:"1px solid rgba(74,222,128,.3)"};
