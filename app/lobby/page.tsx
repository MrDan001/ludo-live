...
type OpenRoom={code:string;players:number;roomSize:number;hostName:string;stake?:number};
export default function LobbyPage(){
 ...
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <section style={{marginTop:28}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{margin:0}}>🔥 Rooms waiting for players</h2><span style={{color:"#64748b",fontSize:12}}>{rooms.length} open</span></div>
   {rooms.length===0?<div style={empty}>No open rooms right now.<br/><span>Create a game and your room will appear here automatically.</span></div>:<div style={{display:"grid",gap:10}}>{rooms.map(room=>{const stake=Math.max(500,Math.min(10000,Number(room.stake)||500));return <div key={room.code} style={roomCard}><div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}><div style={{width:46,height:46,borderRadius:"50%",display:"grid",placeItems:"center",background:"linear-gradient(135deg,#1d4ed8,#22c55e)",fontSize:23}}>🎲</div><div><b style={{fontSize:16}}>{room.hostName}'s Game</b><div style={{color:"#94a3b8",fontSize:12,marginTop:3}}>Room {room.code} · {room.roomSize}-player game</div><div style={{color:"#fbbf24",fontSize:12,fontWeight:900,marginTop:4}}>🪙 {stake.toLocaleString()} coins per player · Pot {((stake*room.roomSize)).toLocaleString()}</div></div></div><div style={{display:"flex",alignItems:"center",gap:12}}><span style={{color:"#4ade80",fontWeight:900,fontSize:13}}>{room.players}/{room.roomSize}</span><button onClick={()=>openRoom(`/room?action=join&code=${room.code}&size=${room.roomSize}&stake=${stake}`)} style={joinBtn}>JOIN</button></div></div>})}</div>}
  </section>
  <div style={flow}><b>Live matchmaking</b><br/><span>Create Room → Choose stake → Room appears online → Players join → Ready → Stakes lock → Host starts → Winner receives the full pot</span></div>
