...
export default function LobbyPage(){
 const [rooms,setRooms]=useState<OpenRoom[]>([]);const [connected,setConnected]=useState(false);const [walletCoins,setWalletCoins]=useState<number|null>(null);const [level,setLevel]=useState(1);const gate=useAccountGate();
 useEffect(()=>{let mounted=true;(async()=>{try{const r=await fetch("/api/auth",{cache:"no-store"});const d=await r.json();if(mounted){setWalletCoins(Number.isFinite(Number(d?.user?.coins))?Number(d.user.coins):null);setLevel(Math.max(1,Number(d?.user?.level)||1))}}catch{}})();const socket:Socket=io(window.location.origin,{transports:["websocket","polling"]});const refresh=(list:OpenRoom[])=>{void hydrateRoomStakes(list).then(next=>{if(mounted)setRooms(next)});};socket.on("connect",()=>{setConnected(true);socket.emit("list-rooms")});socket.on("disconnect",()=>setConnected(false));socket.on("room-list",(list:OpenRoom[])=>refresh(list));return()=>{mounted=false;socket.disconnect()}},[]);
 const openRoom=(href:string)=>{gate.check(()=>{window.location.href=href})};
 const openWorld=()=>{if(level>=5)window.location.href="/ludo-world"};
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <section style={{marginTop:22}}><div style={{marginBottom:12}}><h2 style={{margin:"0 0 4px"}}>🎮 Create or join a game</h2><p style={{margin:0,color:"#64748b",fontSize:13}}>Create a room for friends or join an existing room.</p></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}><button onClick={()=>openRoom("/room?action=create")} style={tile}><span style={{fontSize:30}}>➕</span><b style={{display:"block",fontSize:20,marginTop:8}}>Create Game</b><small>Choose a free room or set a coin stake for your room.</small></button><button onClick={()=>openRoom("/room?action=join")} style={tile}><span style={{fontSize:30}}>🔑</span><b style={{display:"block",fontSize:20,marginTop:8}}>Join by Code</b><small>Enter a friend's private room code.</small></button><button onClick={openWorld} disabled={level<5} style={{...worldTile,opacity:level<5?.72:1,cursor:level<5?"default":"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:30}}>🌎</span><span style={level>=5?worldBadge:lockedBadge}>{level>=5?"UNLOCKED":"LEVEL 5"}</span></div><b style={{display:"block",fontSize:20,marginTop:8}}>Ludo World</b><small>{level>=5?"Enter the new world of ranked battles, powers, teams and tournaments.":"🔒 Reach Level 5 to unlock the entire Ludo World."}</small></button></div></section>
  <section style={{marginTop:28}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{margin:0}}>🔥 Rooms waiting for players</h2><span style={{color:"#64748b",fontSize:12}}>{rooms.length} open</span></div>{rooms.length===0?<div style={empty}>No open rooms right now.<br/><span>Create a game and your room will appear here automatically.</span></div>:<div style={{display:"grid",gap:10}}>{rooms.map(room=>{const paid=room.stakeType==="paid"||!!room.paid&&Number(room.stakeCoins||0)>0;const stake=Math.max(0,Math.trunc(Number(room.stakeCoins||0)));const canAfford=!paid||walletCoins===null||walletCoins>=stake;return <div key={room.code} style={roomCard}><div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}><div style={{width:46,height:46,borderRadius:"50%",display:"grid",placeItems:"center",background:paid?"linear-gradient(135deg,#7c3aed,#f59e0b)":"linear-gradient(135deg,#1d4ed8,#22c55e)",fontSize:23}}>{paid?"🪙":"🎲"}</div><div><div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}><b style={{fontSize:16}}>{room.hostName}'s Game</b><span style={paid?paidBadge:freeBadge}>{paid?`PAID · ${stake.toLocaleString()} COINS`:"FREE"}</span></div><div style={{color:"#94a3b8",fontSize:12,marginTop:3}}>Room {room.code} · {room.roomSize}-player game</div></div></div><div style={{display:"flex",alignItems:"center",gap:12}}><span style={{color:"#4ade80",fontWeight:900,fontSize:13}}>{room.players}/{room.roomSize}</span><button disabled={!canAfford} onClick={()=>openRoom(`/room?action=join&code=${encodeURIComponent(room.code)}&size=${room.roomSize}`)} style={{...joinBtn,opacity:canAfford?1:.45,cursor:canAfford?"pointer":"not-allowed"}}>{paid&&!canAfford?"LOW COINS":"JOIN"}</button></div></div>})}</div>}</section>
...
const worldTile={border:"1px solid rgba(124,92,255,.34)",cursor:"pointer",textAlign:"left" as const,textDecoration:"none",color:"#fff",padding:22,borderRadius:18,background:"radial-gradient(circle at top right,rgba(124,58,237,.22),transparent 46%),linear-gradient(135deg,rgba(19,23,67,.98),rgba(8,20,43,.98))",boxShadow:"0 12px 36px rgba(79,70,229,.1)"};
const worldBadge={padding:"5px 8px",borderRadius:999,background:"rgba(34,197,94,.12)",color:"#86efac",fontSize:9,fontWeight:950,letterSpacing:1};
const lockedBadge={padding:"5px 8px",borderRadius:999,background:"rgba(148,163,184,.1)",color:"#94a3b8",fontSize:9,fontWeight:950,letterSpacing:1};
