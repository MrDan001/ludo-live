...
export default function LobbyPage(){
 const [rooms,setRooms]=useState<OpenRoom[]>([]);const [connected,setConnected]=useState(false);
 ...
 return <AppFrame><div style={{maxWidth:900,margin:"0 auto",paddingBottom:40}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:22}}><Link href="/room?action=create" style={tile}><span style={{fontSize:30}}>➕</span><b style={{display:"block",fontSize:20,marginTop:8}}>Create Game</b><small>Open your own room and wait for players.</small></Link><Link href="/room?action=join" style={tile}><span style={{fontSize:30}}>🔑</span><b style={{display:"block",fontSize:20,marginTop:8}}>Join by Code</b><small>Enter a friend's private room code.</small></Link></div>
