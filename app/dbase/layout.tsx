...
const tools=[
 {label:"🛍️ Shop",match:"shop"},{label:"🧑‍🎨 Avatars",match:"avatars"},{label:"🎯 Missions",match:"missions"},{label:"🎉 Events",match:"events"},{label:"🏆 Tournament Control",match:"tournament"},{label:"🏦 Finance",match:"finance"},
];

export default function DbaseLayout({children}:{children:React.ReactNode}){
 const pathname=usePathname(),router=useRouter(),isLogin=pathname==="/dbase/login";const[checking,setChecking]=useState(!isLogin),[menuOpen,setMenuOpen]=useState(false);
 ...
 const selectTab=(label:string)=>{const button=Array.from(document.querySelectorAll(".dbase-mobile-shell main nav button")).find(x=>x.textContent?.toLowerCase().includes(label.toLowerCase()));if(button){(button as HTMLButtonElement).click();setMenuOpen(false)}};
 const openTool=(match:string)=>{const slot=document.querySelector<HTMLElement>(`.dbase-tool-slot[data-tool="${match}"]`);const button=slot?.querySelector<HTMLButtonElement>("button");if(button){button.click();setMenuOpen(false)}};
 ...
 return <div className={`dbase-mobile-shell${menuOpen?" dbase-menu-open":""}`}>
  ...
  {menuOpen&&<div className="dbase-menu-backdrop" onClick={()=>setMenuOpen(false)}/>}<aside className="dbase-mobile-drawer" aria-hidden={!menuOpen}>
