...
export default function DbaseLogin(){
  ...
  async function submit(e:FormEvent){
    ...
    try{
      const login=await fetch("/api/auth",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({action:"login",identifier,password})});
      ...
      if(!admin.ok){await fetch("/api/auth",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({action:"logout"})});throw new Error("This account does not have DBASE administrator access.");}
      router.replace(next.startsWith("/")&&!next.startsWith("//")?next:"/dbase");
      router.refresh();
    }catch(err){setError(err instanceof Error?err.message:"Unable to sign in.");}
    finally{setBusy(false)}
  }

  return <main style={styles.page}><div style={styles.card}><div style={styles.icon}>🔐</div><div style={styles.eyebrow}>LUDO LIVE • ADMINISTRATION</div><h1 style={styles.title}>DBASE</h1><p style={styles.subtitle}>Secure administrator gateway</p><form onSubmit={submit} style={styles.form}><label style={styles.label}>Admin email or username<input style={styles.input} autoComplete="username" value={identifier} onChange={e=>setIdentifier(e.target.value)} placeholder="admin@example.com" required/></label><label style={styles.label}>Password<input style={styles.input} type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" required/></label>{error&&<div style={styles.error}>{error}</div>}<button disabled={busy} style={{...styles.button,opacity:busy?.7:1}}>{busy?"VERIFYING…":"ENTER DBASE →"}</button></form><p style={styles.note}>Your normal player login does not grant DBASE access. Only authorized administrator accounts can enter.</p></div></main>
}
