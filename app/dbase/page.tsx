...
export default function Dbase(){
 ...
 if(error)return <main style={styles.page}><div style={styles.error}><span>🔐</span><h1>Admin access required</h1><p>{error}</p><Link href="/dbase/login" style={styles.primary}>Admin Login</Link></div></main>;
 if(!data)return <main style={styles.page}><div style={styles.loading}><span>🛡️</span><h1>Loading Admin</h1><p>Securing your command center…</p></div></main>;
 ...
 return <main style={styles.page}><div style={styles.container}>
  ...
  <section style={styles.stats}>{stats.map(x=><div key={x.label} style={styles.stat}><span style={styles.statIcon}>{x.icon}</span><div><small>{x.label}</small><strong>{x.value}</strong></div></div>)}</section>
  <section><div style={styles.sectionTitle}><div><h2>Management</h2><p>Choose a section to manage it on its own page.</p></div></div><div style={styles.grid}>{sections.map(([slug,icon,name,desc])=><Link href={`/dbase/${slug}`} key={slug} style={styles.card}><span style={styles.cardIcon}>{icon}</span><div style={styles.cardBody}><h3>{name}</h3><p>{desc}</p></div><span style={styles.arrow}>→</span></Link>)}</div></section>
  <section style={styles.quick}><div><h2>Admin information</h2><p>Existing players, shop data, rewards, inventory, economy records, support records and other stored information remain untouched by this interface rebuild.</p></div><span>🛡️</span></section>
 </div></main>
}
const styles:any={page:{minHeight:"100vh",background:"#030817",color:"#eef5ff",padding:"20px 14px 60px",fontFamily:"system-ui,-apple-system,sans-serif"},container:{width:"min(1120px,100%)",margin:"0 auto"},header:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,padding:"10px 2px 22px"},kicker:{fontSize:10,fontWeight:900,letterSpacing:2,color:"#70b0ff"},title:{fontSize:"clamp(28px,6vw,42px)",margin:"5px 0 2px",lineHeight:1.05},subtitle:{margin:0,color:"#8298b8",fontSize:13,overflowWrap:"anywhere"},status:{padding:"8px 11px",borderRadius:999,background:"#0b2b20",border:"1px solid #1c6848",color:"#79e6ac",fontWeight:900,fontSize:11},stats:{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:10,marginBottom:28},stat:{display:"flex",alignItems:"center",gap:10,minWidth:0,padding:14,borderRadius:16,background:"#08142b",border:"1px solid #1c3356"},statIcon:{fontSize:22},stat:{display:"flex",alignItems:"center",gap:10,minWidth:0,padding:14,borderRadius:16,background:"#08142b",border:"1px solid #1c3356"},sectionTitle:{marginBottom:12},sectionTitle h2:{},grid:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12},card:{display:"flex",alignItems:"center",gap:13,minWidth:0,textDecoration:"none",color:"inherit",padding:16,borderRadius:16,background:"#08142b",border:"1px solid #1c3356",transition:"transform .15s"},cardIcon:{fontSize:28,width:42,textAlign:"center"},cardBody:{minWidth:0,flex:1},cardBody h3:{margin:"0 0 4px",fontSize:16},cardBody p:{margin:0,color:"#8298b8",fontSize:12,lineHeight:1.4},arrow:{fontSize:22,color:"#6ea8ef"},quick:{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",marginTop:28,padding:17,borderRadius:16,background:"#071125",border:"1px solid #182f50"},quick span:{fontSize:30},loading:{minHeight:"80vh",display:"grid",placeItems:"center",alignContent:"center",textAlign:"center"},loading span:{fontSize:42},error:{minHeight:"80vh",display:"grid",placeItems:"center",alignContent:"center",textAlign:"center",gap:8},error span:{fontSize:42},primary:{display:"inline-block",padding:"11px 16px",borderRadius:11,background:"#1769e8",color:"white",textDecoration:"none",fontWeight:900}};
