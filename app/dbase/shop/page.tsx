...
export default function AdminShopPage() {
  ...
  return <div className="shop-admin">
    ...
    {loading ? <div className="shop-loading">Loading live shop catalogue…</div> : <div className="shop-grid">{filtered.map((item, index) => {
      ...
      return <article className="shop-card" key={`${item.type}-${item.id}-${index}`}><div className="card-image"><Visual item={item} /><span className={`rarity ${String(item.rarity || "COMMON").toLowerCase()}`}>{item.rarity || "COMMON"}</span>{hidden && <span className="hidden-badge">HIDDEN</span>}</div><div className="card-body"><div className="card-name"><h3>{item.name || item.id}</h3><span>{item.description || item.yardKind || item.category || item.type}</span></div><div className="card-price"><b>{priceLabel(item)}</b></div><div className="card-actions"><button onClick={() => openEdit(item)}>Edit</button>{isAvatar ? <><button onClick={() => avatarAction(String(item.id), hidden ? "show_avatar" : "hide_avatar")}>{hidden ? "Show" : "Hide"}</button>{!item.isBuiltin && <button className="danger" onClick={() => avatarAction(String(item.id), "delete_avatar")}>Delete</button>}</> : item.isVisual ? <button className="danger" onClick={async () => { if (!window.confirm("Delete this item?")) return; await fetch("/api/admin/shop", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "toggle_visual", id: item.id }) }); await load(); }}>Delete</button> : null}</div></div></article>;
    })}</div>}
    {editing && <div className="editor-backdrop" onMouseDown={(event) => event.currentTarget === event.target && setEditing(null)}><div className="shop-editor"><div className="editor-title"><div><span>{editing.isNewAvatar ? "AVATAR SHOP ITEM" : "CATALOG ITEM"}</span><h2>{editing.id ? "Edit item" : "Add item"}</h2></div><button onClick={() => setEditing(null)}>✕</button></div><div className="editor-preview"><Visual item={editing} /><div><b>{editing.name || "New item"}</b></div></div><div className="form-grid"><label>Name<input value={editing.name || ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })} /></label><label className="wide">Description<textarea value={editing.description || ""} onChange={(e) => setEditing({ ...editing, description: e.target.value })} /></label>{editing.isNewAvatar && <div className="wide image-method"><strong>Avatar image</strong><div className="image-mode-tabs"><button type="button" className={imageMode === "upload" ? "active" : ""} onClick={() => { setImageMode("upload"); setEditing({ ...editing, imageUrl: "" }); }}>🖼️ Upload Image</button><button type="button" className={imageMode === "url" ? "active" : ""} onClick={() => { setImageMode("url"); setEditing({ ...editing, imageData: "" }); }}>🔗 Image URL</button></div>{imageMode === "upload" ? <><input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden onChange={(e) => handleFile(e.target.files?.[0])} /><button type="button" className="add-button" onClick={() => fileRef.current?.click()}>{fileName ? "Change Image" : "Choose Image"}</button>{fileName && <small>{fileName} · max 5 MB</small>}</> : <input value={editing.imageUrl || ""} onChange={(e) => setEditing({ ...editing, imageUrl: e.target.value, imageData: "" })} placeholder="https://example.com/avatar.png" />}</div>}{editing.isNewAvatar && <label>Avatar Category<select value={editing.categoryId || "classic"} onChange={(e) => setEditing({ ...editing, categoryId: e.target.value })}>{AVATAR_CATEGORIES.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>}<label>Currency<select value={editing.currency || "coins"} onChange={(e) => setEditing({ ...editing, currency: e.target.value })}><option value="coins">Coins</option><option value="gems">Gems</option><option value="naira">Naira</option></select></label><label>Price<input type="number" min="0" value={editing.price ?? 0} onChange={(e) => setEditing({ ...editing, price: Number(e.target.value) })} /></label><label>Unlock level<input type="number" min="0" value={editing.requiredLevel ?? 0} onChange={(e) => setEditing({ ...editing, requiredLevel: Number(e.target.value) })} /></label><label>Rarity<select value={editing.rarity || "COMMON"} onChange={(e) => setEditing({ ...editing, rarity: e.target.value })}><option>COMMON</option><option>RARE</option><option>EPIC</option><option>LEGENDARY</option></select></label></div><button className="save-button" disabled={saving} onClick={save}>{saving ? "Saving…" : "Save shop changes"}</button></div></div>}
  </div>;
}
