...
const priceLabel = (item: Item) => item.currency === "naira" ? `₦${money(item.price)}` : `${item.currency === "gems" ? "💎" : "🪙"} ${money(item.price)}`;
...
export default function AdminShopPage() {
  ...
  const openNew = (kind: AddKind) => {
    ...
    setImageMode("upload");
    if (kind === "avatar") {
      setEditing({ isNewAvatar: true, name: "New Avatar", description: "", categoryId: "classic", rarity: "COMMON", currency: "gems", price: 500, requiredLevel: 0, imageUrl: "", imageData: "", isActive: true });
      return;
    }
    if (kind === "yard-background" || kind === "yard-backgroundless") {
      const backgroundless = kind === "yard-backgroundless";
      setEditing({ isVisual: true, name: backgroundless ? "New Backgroundless Yard" : "New Background Yard", description: "", kind: "background", yardKind: backgroundless ? "backgroundless" : "background", backgroundless, hasBackground: !backgroundless, rarity: "COMMON", currency: "gems", price: 50, stockQuantity: -1, requiredLevel: 0, icon: "🏠", imageUrl: "" });
      return;
    }
  ...
  const openEdit = (item: Item) => {
    setFileName("");
    setImageMode(item.imageData ? "upload" : "url");
  ...
  const handleFile = (file?: File) => {
    ...
    reader.onload = () => {
      ...
      setEditing((current) => current ? { ...current, imageData: String(reader.result), imageUrl: "" } : current);
      setMessage("");
    };
  ...
  const avatarAction = async (id: string, action: "hide_avatar" | "show_avatar" | "delete_avatar") => {
    ...
    try {
      ...
      setMessage(action === "delete_avatar" ? "Avatar deleted." : action === "hide_avatar" ? "Avatar hidden from players." : "Avatar is visible again.");
      await load();
    } catch (error: any) {
      setMessage(error?.message || "Avatar action failed");
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = editing.isNewAvatar
        ? { ...editing, action, categoryId: editing.categoryId || "classic", imageUrl: imageMode === "url" ? String(editing.imageUrl || "").trim() : "", imageData: imageMode === "upload" ? editing.imageData : "" }
        : editing.isVisual ? { ...editing, action } : { ...editing, action: "save", isActive: editing.isActive !== false };
  ...
  return <div className="shop-admin">
    <div className="shop-topbar"><div className="shop-title"><Link href="/dbase" className="back">← Dashboard</Link><div className="eyebrow">LIVE SHOP CONTROL</div><h1>Shop Management</h1><p>Manage pricing, stock, categories and visuals for everything players can buy.</p></div><div className="add-actions"><button className="add-button" onClick={() => openNew("avatar")}>＋ Avatar</button><button className="add-button" onClick={() => openNew("yard-background")}>＋ Background Yard</button><button className="add-button" onClick={() => openNew("yard-backgroundless")}>＋ Backgroundless Yard</button><button className="add-button" onClick={() => openNew("sticker")}>＋ Sticker</button></div></div>
