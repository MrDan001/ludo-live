...
function ConfirmModal({ title, body, icon, busy, onCancel, onConfirm }: { title: string; body: string; icon: string; busy?: boolean; onCancel: () => void; onConfirm: () => void }) {
  return <div className="editor-backdrop" style={{ zIndex: 100000 }} onMouseDown={(e) => e.currentTarget === e.target && !busy && onCancel()}>
    <div style={{ width: "min(460px,calc(100vw - 32px))", borderRadius: 28, padding: 28, color: "#fff", background: "linear-gradient(145deg,#111c31,#071225)", border: "1px solid rgba(255,255,255,.10)", boxShadow: "0 30px 100px rgba(0,0,0,.65)" }}>
      <div style={{ width: 62, height: 62, borderRadius: 20, display: "grid", placeItems: "center", fontSize: 30, background: "rgba(248,113,113,.12)", border: "1px solid rgba(248,113,113,.20)" }}>{icon}</div>
      <div style={{ marginTop: 18, fontSize: 11, fontWeight: 900, letterSpacing: 2, color: "#fca5a5" }}>CONFIRM ACTION</div>
      <h2 style={{ margin: "8px 0 8px", fontSize: 25 }}>{title}</h2>
      <p style={{ margin: 0, lineHeight: 1.6, color: "rgba(255,255,255,.68)" }}>{body}</p>
      <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
        <button className="admin-btn" style={{ flex: 1 }} disabled={busy} onClick={onCancel}>Cancel</button>
        <button className="admin-btn danger-btn" style={{ flex: 1 }} disabled={busy} onClick={onConfirm}>{busy ? "Working…" : "Continue"}</button>
      </div>
    </div>
  </div>;
}
...
export default function AdminShopPage() {
  ...
  const [confirm, setConfirm] = useState<{ title: string; body: string; icon: string; action: () => Promise<void> } | null>(null);
  ...
  const openNew = (kind: AddKind) => {
    ...
    if (kind === "avatar") { setEditing({ isNewAvatar: true, name: "New Avatar", description: "", categoryId: "classic", rarity: "COMMON", currency: "gems", price: 500, requiredLevel: 0, imageUrl: "", imageData: "", isActive: true }); return; }
    const backgroundless = kind === "yard-backgroundless";
    if (kind === "yard-background" || backgroundless) { setEditing({ isVisual: true, name: backgroundless ? "New Backgroundless Yard" : "New Background Yard", description: "", kind: "background", yardKind: backgroundless ? "backgroundless" : "background", backgroundless, hasBackground: !backgroundless, rarity: "COMMON", currency: "gems", price: 50, stockQuantity: -1, requiredLevel: 0, icon: "🏠", imageUrl: "" }); return; }
  ...
  const openEdit = (item: Item) => { setFileName(""); setImageMode(item.imageData ? "upload" : "url"); setEditing({ ...item, isVisual: Boolean(item.isVisual), isNewAvatar: item.type === "avatar", imageData: item.imageData || "" }); };
  ...
  const runConfirm = async () => {
    if (!confirm) return;
    setConfirmBusy(true);
    try { await confirm.action(); setConfirm(null); } catch (error: any) { setMessage(error?.message || "Action failed."); }
  ...
  const avatarAction = async (id: string, action: "hide_avatar" | "show_avatar" | "delete_avatar") => {
    ...
    if (action === "delete_avatar") {
      setConfirm({ title: "Delete this avatar?", body: "This will permanently remove the avatar from the admin catalogue. Built-in avatars remain protected.", icon: "🗑️", action: async () => {
        ...
        setMessage("Avatar deleted successfully."); await load();
      }});
      return;
    }
    setMessage("");
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Avatar action failed");
      setMessage(action === "hide_avatar" ? "Avatar hidden from players." : "Avatar is visible again."); await load();
    } catch (error: any) { setMessage(error?.message || "Avatar action failed"); }
  };

  const deleteVisual = (id: string) => setConfirm({ title: "Delete this shop item?", body: "The item will be removed from the active visual catalogue and will no longer be offered to players.", icon: "🗑️", action: async () => {
    const response = await fetch("/api/admin/shop", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "toggle_visual", id }) });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.error || "Unable to remove item");
    setMessage("Shop item removed successfully."); await load();
  }});
  ...
  const save = async () => {
    ...
    try {
      if (isPackage(editing)) {
        const reward = Math.trunc(Number(editing.reward)); const price = Math.trunc(Number(editing.price));
        const rewardCurrency = editing.type === "gem_package" ? "gems" : "coins"; const paymentCurrency = editing.type === "gem_package" ? "naira" : "gems";
      ...
      let action = "save"; if (editing.isNewAvatar) action = editing.id ? "edit_avatar" : "create_avatar"; else if (editing.isVisual) action = editing.id ? "edit_visual" : "create_visual";
  ...
  return <div className="shop-admin">
    ...
    {confirm && <ConfirmModal title={confirm.title} body={confirm.body} icon={confirm.icon} busy={confirmBusy} onCancel={() => setConfirm(null)} onConfirm={runConfirm} />}
  </div>;
}
