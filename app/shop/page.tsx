...
function readWallet(): Wallet {
  try { const stored = JSON.parse(localStorage.getItem("ludo-wallet") || "null"); return { ...defaults, ...(stored || {}) }; }
...
export default function ShopPage() {
  ...
  const [pendingGemPackage, setPendingGemPackage] = useState<(typeof gemPackages)[number] | null>(null);
  ...
  useEffect(() => {
    ...
    const payment = params.get("payment"); const gems = Number(params.get("gems") || 0); const reference = params.get("reference") || "";
    if (payment === "success" && gems > 0 && reference) {
      const appliedKey = `ludo-payment-applied-${reference}`;
      if (!localStorage.getItem(appliedKey)) {
        const current = readWallet(); const next = { ...current, gems: current.gems + gems };
        writeWallet(next); localStorage.setItem(appliedKey, "1"); setWallet(next);
        setNotice(`Payment confirmed. ${gems.toLocaleString()} gems have been added to your wallet.`);
      }
      window.history.replaceState({}, "", "/shop");
    } else if (payment === "failed") {
      setNotice("Payment was not completed. No gems were added."); window.history.replaceState({}, "", "/shop");
    }
  ...
  const buyGemPackage = async () => {
    if (!pendingGemPackage || !email.trim()) return;
    setBusy(true); setNotice("");
    try {
      localStorage.setItem("ludo-paystack-email", email.trim());
      const response = await fetch("/api/paystack/initialize", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ packageId: pendingGemPackage.id, email: email.trim() }) });
  ...
  const confirmPurchase = () => {
    ...
    const next = { ...wallet, gems: wallet.gems - confirm.price, ...(confirm.kind === "coins" ? { coins: wallet.coins + (confirm.quantity || 0) } : {}) };
    writeWallet(next); setWallet(next);
    if (confirm.kind !== "coins") {
      const nextOwned = Array.from(new Set([...owned, confirm.id])); localStorage.setItem("ludo-inventory", JSON.stringify(nextOwned)); setOwned(nextOwned);
    }
...
const label: React.CSSProperties = { display: "grid", gap: 7, color: "#bfdbfe", fontWeight: 800, marginTop: 16 };
const input: React.CSSProperties = { width: "100%", boxSizing: "border-box", padding: 13, borderRadius: 11, border: "1px solid #315b9f", background: "#041128", color: "#fff", outline: "none" };
const modalActions: React.CSSProperties = { display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 20, flexWrap: "wrap" };
const cancelBtn: React.CSSProperties = { border: 0, borderRadius: 11, padding: "13px 16px", background: "#334155", color: "#fff", fontWeight: 900, cursor: "pointer" };
