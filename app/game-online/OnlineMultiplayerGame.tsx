...
const uniquePlayers=(items:Player[])=>{const seen=new Set<string>();return items.filter(item=>{const id=String(item?.playerId||"");if(!id||seen.has(id))return false;seen.add(id);return true;});};

function PlayerAvatar({src}:{src?:string}){
  if(src&&(src.startsWith("http")||src.startsWith("/")||src.startsWith("data:"))) return <img src={src} alt="Profile avatar" className="mp-avatar-image"/>;
  return <span>👤</span>;
}
...
function ChatPanel({open,messages,me,value,setValue,onSend,onClose,onReaction}:{open:boolean;messages:ChatMessage[];me:string;value:string;setValue:(v:string)=>void;onSend:(v?:string)=>void;onClose:()=>void;onReaction:(v:string)=>void}){
  ...
  return <div className="mp-chat-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}>
    <section className="mp-chat-panel" role="dialog" aria-modal="true" aria-label="Room Chat" onMouseDown={e=>e.stopPropagation()}>
      ...
      <div ref={listRef} className="mp-chat-list" style={{flex:"1 1 auto",minHeight:0,height:0,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
...
export default function OnlineMultiplayerGame(){
  ...
  const saveGameCache=useCallback((next:GameState)=>{try{sessionStorage.setItem(cacheKey,JSON.stringify(next));}catch{}},[cacheKey]);
  ...
  const loadProfile=useCallback(async()=>{try{
    ...
    const next:Cosmetics={board:String(customization.equippedBoard||"classic") as BoardThemeId,dice:String(customization.equippedDice||"classic") as DiceSkinId,yard:Array.isArray(customization.equippedItems)?String(customization.equippedItems.find(x=>typeof x==="string"&&x.startsWith("yard-"))||""):""};
    setProfile({name,avatar,level,coins});return{playerId:String(auth?.user?.id||""),name,avatar,level,coins,cosmetics:next};
  }catch{return null;}},[]);

  const loadChatHistory=useCallback(async()=>{try{const response=await fetch(`/api/multiplayer-chat?roomCode=${encodeURIComponent(roomCode)}`,{cache:"no-store"});if(!response.ok)return;const data=await response.json();const history:ChatMessage[]=Array.isArray(data?.messages)?data.messages.map((m:any)=>({id:String(m.id),playerId:String(m.playerId||""),name:String(m.name||"Player"),text:String(m.text||""),at:Number(m.at)||Date.now()})):[];setChatMessages(current=>{const merged=[...current];for(const message of history){if(!merged.some(existing=>existing.id===message.id))merged.push(message);}return merged.sort((a,b)=>a.at-b.at).slice(-100);});}catch{}},[roomCode]);
  ...
  useEffect(()=>{try{const raw=sessionStorage.getItem(cacheKey);if(!raw)return;const cached=JSON.parse(raw) as GameState;if(!cached?.players?.length)return;revisionRef.current=Number(cached.stateRevision??-1);setGame(cached);setPending(cached.pendingMove);if(cached.dice!==null)setRoll(cached.dice);const cachedTokens=normalizeTokens(cached.tokens);authoritativeRef.current=cachedTokens;setTokens(cachedTokens.map(t=>({...t})));}catch{}},[cacheKey]);

  useEffect(()=>{
    let cancelled=false;let s:Socket|null=null;let cosmeticsTimer:number|null=null;
    const start=async()=>{const p=await loadProfile();if(cancelled||!p?.playerId)return;setMe(p.playerId);
      ...
      const join=()=>{setConnected(true);setConnectionMessage("Connected");s?.emit("join-room",{roomCode,roomSize,name:p.name,avatar:p.avatar,level:p.level,coins:p.coins,board:p.cosmetics.board,dice:p.cosmetics.dice,yard:p.cosmetics.yard,playerId:p.playerId});window.setTimeout(()=>{if(mountedRef.current)void loadChatHistory();},80);};
      s.on("connect",join);
      s.on("disconnect",()=>{setConnected(false);clearRollRequest();moveRequestRef.current=false;setAnimating(false);animationRef.current=false;setConnectionMessage("Reconnecting…");});
      s.on("connect_error",error=>{setConnected(false);setConnectionMessage(`Connection error: ${error.message||"retrying"}`);});
      s.on("roster",(members:Player[])=>{const next=uniquePlayers(Array.isArray(members)?members:[]);setRoster(next);const host=next.find(m=>m.host);const mineMember=next.find(m=>String(m.playerId)===String(p.playerId));isHostRef.current=!!mineMember?.host;if(host){const c:Cosmetics={board:String(host.board||"classic") as BoardThemeId,dice:String(host.dice||"classic") as DiceSkinId,yard:String(host.yard||"")};hostCosmeticsRef.current=c;setCosmetics(c);}});
      ...
      s.on("game-state",(next:GameState)=>{if(!mountedRef.current)return;const revision=Number(next.stateRevision??-1);if(revision>=0&&revisionRef.current>=0&&revision<revisionRef.current)return;if(revision>=0)revisionRef.current=revision;setGame(next);const mineTurn=String(next.currentPlayerId||"")===String(p.playerId);setPending(mineTurn?next.pendingMove:null);if(next.dice!==null)setRoll(next.dice);const authoritative=normalizeTokens(next.tokens||{});authoritativeRef.current=authoritative;if(!animationRef.current&&moveQueueRef.current.length===0)setTokens(authoritative.map(t=>({...t})));saveGameCache(next);if(next.status!=="playing"){clearRollRequest();moveRequestRef.current=false;setAnimating(false);animationRef.current=false;}});
      s.on("game-dice",(event:{value:DiceValue;playerId?:string})=>{if(Number(event?.value)>=1)setRoll(event.value);if(String(event?.playerId||"")!==String(p.playerId)){setRemoteRolling(true);window.setTimeout(()=>mountedRef.current&&setRemoteRolling(false),500);}else{setRemoteRolling(false);clearRollRequest();}});
      ...
      s.on("game-move-error",()=>{moveRequestRef.current=false;setActionBusy(false);animationRef.current=false;setAnimating(false);recover(s);});
      ...
      cosmeticsTimer=window.setInterval(async()=>{if(!mountedRef.current||!isHostRef.current)return;try{const response=await fetch("/api/customization",{cache:"no-store"});if(!response.ok)return;const cData:CustomizationResponse=await response.json();const c:Cosmetics={board:String(cData.equippedBoard||"classic") as BoardThemeId,dice:String(cData.equippedDice||"classic") as DiceSkinId,yard:Array.isArray(cData.equippedItems)?String(cData.equippedItems.find(x=>typeof x==="string"&&x.startsWith("yard-"))||""):""};if(c.board!==hostCosmeticsRef.current.board||c.dice!==hostCosmeticsRef.current.dice||c.yard!==hostCosmeticsRef.current.yard){hostCosmeticsRef.current=c;setCosmetics(c);s?.emit("host-cosmetics-update",c);}}catch{}},1200);
    ...
    };void start();return()=>{cancelled=true;if(cosmeticsTimer!==null)window.clearInterval(cosmeticsTimer);s?.disconnect();};
  },[roomCode,roomSize,loadProfile,loadChatHistory,recover,saveGameCache,clearRollRequest]);
  ...
  const animateMove=useCallback(async(move:MoveEvent)=>{
    ...
    const update=(position:number)=>setTokens(prev=>prev.map(t=>t.color===color&&t.id===id?{...t,position,state:tokenState(position)}:t));update(from);
    if(Number.isFinite(contact)&&contact!==from){const step=contact>from?1:-1;for(let position=from+step;;position+=step){await new Promise(resolve=>window.setTimeout(resolve,145));if(!mountedRef.current)return;update(position);if(position===contact)break;}}
    if(move.captureToCenter&&move.captured){setTokens(prev=>prev.map(t=>t.color===move.captured?.color&&t.id===move.captured?.id?{...t,position:0,state:"yard"}:t));await new Promise(resolve=>window.setTimeout(resolve,150));}
    if(finalTo!==contact&&Number.isFinite(finalTo)){await new Promise(resolve=>window.setTimeout(resolve,150));if(!mountedRef.current)return;update(finalTo);}
  ...
  const legalTokenKeys=useMemo(()=>{if(!pending||!myTurn)return[];const source=authoritativeRef.current;return source.filter(t=>myColors.includes(t.color)&&canMove(source,t,pending)).map(t=>`${t.color}-${t.id}`);},[pending,myTurn,myColors,game?.stateRevision]);

  const chooseToken=useCallback((color:Color,id:number)=>{if(!socket?.connected||!game||!myTurn||pending===null||moveRequestRef.current)return;const source=authoritativeRef.current;const token=source.find(t=>t.color===color&&t.id===id);if(!token||!myColors.includes(color)||!canMove(source,token,pending))return;moveRequestRef.current=true;setActionBusy(true);socket.emit("game-move",{tokenId:`${color}:${id}`},(ack:{ok?:boolean})=>{if(!ack?.ok){moveRequestRef.current=false;setActionBusy(false);recover(socket);}});},[socket,game,myTurn,pending,myColors,recover]);

  const handleRoll=useCallback(()=>{if(!socket?.connected||!game||!myTurn||pending!==null||remoteRolling||rollRequestRef.current||actionBusy)return;rollRequestRef.current=true;setActionBusy(true);if(rollTimerRef.current!==null)window.clearTimeout(rollTimerRef.current);rollTimerRef.current=window.setTimeout(()=>{clearRollRequest();if(socket.connected)recover(socket);},5000);socket.emit("game-roll",(ack:{ok?:boolean})=>{if(!ack?.ok)clearRollRequest();});},[socket,game,myTurn,pending,remoteRolling,actionBusy,recover,clearRollRequest]);
  ...
  const diceDisabled=!myTurn||pending!==null||remoteRolling||!connected||game?.status!=="playing"||rollRequestRef.current||moveRequestRef.current;
