...
function ChatPanel({open,messages,me,value,setValue,onSend,onClose,onReaction}:{open:boolean;messages:ChatMessage[];me:string;value:string;setValue:(v:string)=>void;onSend:(v?:string)=>void;onClose:()=>void;onReaction:(v:string)=>void}){
  ...
  return <div className="mp-chat-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}>
    <section className="mp-chat-panel" role="dialog" aria-modal="true" aria-label="Room Chat" onMouseDown={e=>e.stopPropagation()} style={{height:"min(84vh,720px)"}}>
...
export default function OnlineMultiplayerGame(){
  ...
  const loadProfile=useCallback(async()=>{try{const [authResponse,customizationResponse]=await Promise.all([fetch("/api/auth",{cache:"no-store"}),fetch("/api/customization",{cache:"no-store"})]);const auth=await authResponse.json();const customization:CustomizationResponse=customizationResponse.ok?await customizationResponse.json():{};const name=String(auth?.user?.username||"Player");const avatar=String(auth?.user?.avatar||auth?.user?.image||"");const level=Math.max(1,Number(auth?.user?.level)||1);const coins=Math.max(0,Number(auth?.user?.coins)||0);const cosmetics:Cosmetics={board:String(customization.equippedBoard||"classic") as BoardThemeId,dice:String(customization.equippedDice||"classic") as DiceSkinId,yard:Array.isArray(customization.equippedItems)?String(customization.equippedItems.find(x=>typeof x==="string"&&x.startsWith("yard-"))||""):""};setProfile({name,avatar,level,coins});return{playerId:String(auth?.user?.id||""),name,avatar,level,coins,cosmetics};}catch{return null;}},[]);
  ...
  useEffect(()=>{let cancelled=false;let s:Socket|null=null;let cosmeticsTimer:number|null=null;const start=async()=>{const p=await loadProfile();if(cancelled||!p?.playerId)return;setMe(p.playerId);setCosmetics(p.cosmetics);hostCosmeticsRef.current=p.cosmetics;s=io(window.location.origin,{transports:["websocket"],reconnection:true,reconnectionAttempts:Infinity,reconnectionDelay:250,reconnectionDelayMax:1500});setSocket(s);const join=()=>{if(cancelled)return;setConnected(true);setConnectionMessage("Connected");s?.emit("join-room",{roomCode,roomSize,name:p.name,avatar:p.avatar,level:p.level,coins:p.coins,board:p.cosmetics.board,dice:p.cosmetics.dice,yard:p.cosmetics.yard,playerId:p.playerId});void loadChatHistory();};s.on("connect",join);s.on("disconnect",()=>{setConnected(false);clearRollRequest();moveRequestRef.current=false;setConnectionMessage("Reconnecting…");});s.on("connect_error",error=>{setConnected(false);setConnectionMessage(`Connection error: ${error.message||"retrying"}`);});s.on("roster",(members:Player[])=>{const next=uniquePlayers(Array.isArray(members)?members:[]);setRoster(next);const host=next.find(m=>m.host);const mine=next.find(m=>String(m.playerId)===String(p.playerId));isHostRef.current=!!mine?.host;if(host){const c:Cosmetics={board:String(host.board||"classic") as BoardThemeId,dice:String(host.dice||"classic") as DiceSkinId,yard:String(host.yard||"")};hostCosmeticsRef.current=c;setCosmetics(c);}});s.on("host-cosmetics",(value:Cosmetics)=>{const c:Cosmetics={board:String(value?.board||"classic") as BoardThemeId,dice:String(value?.dice||"classic") as DiceSkinId,yard:String(value?.yard||"")};hostCosmeticsRef.current=c;setCosmetics(c);});s.on("game-state",(next:GameState)=>{if(!mountedRef.current)return;const authoritative=normalizeTokens(next.tokens||{});authoritativeRef.current=authoritative;setGame(next);setTokens(current=>animationRef.current?current:authoritative.map(t=>({...t})));const mineTurn=String(next.currentPlayerId||"")===String(p.playerId);if(!mineTurn||next.pendingMove===null)moveRequestRef.current=false;if(mineTurn&&next.pendingMove!==null)rollRequestRef.current=false;setPendingMoveRef(next.pendingMove,mineTurn);if(next.dice!==null)setRoll(next.dice);if(next.status!=="playing"){clearRollRequest();moveRequestRef.current=false;setAnimating(false);animationRef.current=false;}});s.on("game-dice",(event:{value:DiceValue;playerId?:string})=>{if(Number(event?.value)>=1)setRoll(event.value);setRemoteRolling(true);window.setTimeout(()=>mountedRef.current&&setRemoteRolling(false),900);if(String(event?.playerId||"")===String(p.playerId))clearRollRequest();});s.on("game-moved",(move:MoveEvent)=>{if(!move?.tokenId)return;moveRequestRef.current=false;setActionBusy(false);moveQueueRef.current.push(move);if(!animationRef.current){const next=moveQueueRef.current.shift();if(next)void animateRef.current(next);}});s.on("game-roll-error",()=>clearRollRequest());s.on("game-move-error",()=>{moveRequestRef.current=false;setActionBusy(false);recover(s);});s.on("chat",(message:any)=>{if(!message?.text)return;const normalized:ChatMessage={id:String(message.id||`${message.playerId||"player"}-${message.at||Date.now()}`),playerId:String(message.playerId||""),name:String(message.name||"Player"),text:String(message.text),at:Number(message.at)||Date.now()};setChatMessages(items=>items.some(item=>item.id===normalized.id)?items:[...items.slice(-99),normalized]);if(!chatOpenRef.current&&String(message.playerId)!==String(p.playerId))setChatUnread(true);});s.on("game-finished",(event:any)=>setConnectionMessage(String(event?.winnerId||"")===String(p.playerId)?"You won!":"Match finished"));const onOnline=()=>{if(s?.connected)recover(s);};window.addEventListener("online",onOnline);cosmeticsTimer=window.setInterval(async()=>{if(!mountedRef.current||!isHostRef.current)return;try{const response=await fetch("/api/customization",{cache:"no-store"});if(!response.ok)return;const data:CustomizationResponse=await response.json();const c:Cosmetics={board:String(data.equippedBoard||"classic") as BoardThemeId,dice:String(data.equippedDice||"classic") as DiceSkinId,yard:Array.isArray(data.equippedItems)?String(data.equippedItems.find(x=>typeof x==="string"&&x.startsWith("yard-"))||""):""};if(c.board!==hostCosmeticsRef.current.board||c.dice!==hostCosmeticsRef.current.dice||c.yard!==hostCosmeticsRef.current.yard){hostCosmeticsRef.current=c;setCosmetics(c);s?.emit("host-cosmetics-update",c);}}catch{}},1000);return()=>{window.removeEventListener("online",onOnline);if(cosmeticsTimer!==null)window.clearInterval(cosmeticsTimer);s?.disconnect();};};void start();return()=>{cancelled=true;if(cosmeticsTimer!==null)window.clearInterval(cosmeticsTimer);s?.disconnect();};},[roomCode,roomSize,loadProfile,loadChatHistory,recover,clearRollRequest]);
  const [pendingMove,setPendingMove]=useState<DiceValue|null>(null);const setPendingMoveRef=(value:DiceValue|null,mine:boolean)=>setPendingMove(mine?value:null);
  const animateMove=useCallback(async(move:MoveEvent)=>{if(!mountedRef.current)return;animationRef.current=true;setAnimating(true);const [color,idText]=String(move.tokenId).split(":");const id=Number(idText);if(!COLORS.includes(color as Color)||!Number.isInteger(id)){animationRef.current=false;setAnimating(false);return;}const from=Number(move.from);const contact=Number(move.captureToCenter?(move.captureProgress??move.to??move.target):(move.to??move.target));const finalTo=Number(move.finalTo??(move.captureToCenter?FINISH_PROGRESS:contact));const update=(position:number)=>setTokens(prev=>prev.map(t=>t.color===color&&t.id===id?{...t,position,state:tokenState(position)}:t));update(from);if(Number.isFinite(contact)&&contact!==from){const step=contact>from?1:-1;for(let position=from+step;;position+=step){await new Promise(resolve=>window.setTimeout(resolve,280));if(!mountedRef.current)return;update(position);if(position===contact)break;}}if(move.captureToCenter&&move.captured){setTokens(prev=>prev.map(t=>t.color===move.captured?.color&&t.id===move.captured?.id?{...t,position:0,state:"yard"}:t));await new Promise(resolve=>window.setTimeout(resolve,280));}if(finalTo!==contact&&Number.isFinite(finalTo)){await new Promise(resolve=>window.setTimeout(resolve,280));if(!mountedRef.current)return;update(finalTo);}const next=moveQueueRef.current.shift();if(next){await animateRef.current(next);return;}animationRef.current=false;setAnimating(false);setTokens(authoritativeRef.current.map(t=>({...t})));},[]);
  useEffect(()=>{animateRef.current=animateMove;},[animateMove]);
  const players=useMemo(()=>mergePlayers(game?.players||[],roster),[game?.players,roster]);const mine=players.find(p=>String(p.playerId)===String(me))||players[0];const opponent=players.find(p=>String(p.playerId)!==String(me));const myColors=useMemo<Color[]>(()=>mine?.colors?.length?mine.colors:(playerColorsForSeats(roomSize,Number(mine?.seat)||0) as Color[]),[mine,roomSize]);const myTurn=!!game&&connected&&game.status==="playing"&&String(game.currentPlayerId||"")===String(me);const legalTokenKeys=useMemo(()=>{if(pendingMove===null||!myTurn)return[];const source=authoritativeRef.current;return source.filter(t=>myColors.includes(t.color)&&canMove(source,t,pendingMove)).map(t=>`${t.color}-${t.id}`);},[pendingMove,myTurn,myColors,game?.stateRevision]);
  ...
  const statusText=game?.status==="finished"?(game.winnerId===me?"YOU WON":"MATCH FINISHED"):myTurn?"YOUR TURN":"OPPONENT'S TURN";const meName=mine?.name||profile.name;const opponentName=opponent?.name||"Opponent";const commMembers=players.map(player=>({id:String(player.playerId),playerId:String(player.playerId),name:player.name,role:player.host?"host":"member",online:player.connected!==false,peerId:player.peerId}));const diceDisabled=!myTurn||pendingMove!==null||!connected||game?.status!=="playing"||rollRequestRef.current||moveRequestRef.current;
  return <main className="mp-page"><div className="mp-shell"><header className="mp-header"><div className="mp-player-card"><div className="mp-avatar"><PlayerAvatar src={mine?.avatar||profile.avatar}/></div><div className="mp-player-copy"><strong>{meName} <span>YOU</span></strong><small className="mp-player-level">⭐ {mine?.level??profile.level}</small><small><i className="mp-online-dot"/> Connected</small></div></div><div className="mp-brand"><strong>♛ LUDO</strong><span>LIVE</span></div><div className="mp-player-card mp-player-card-opponent"><div className="mp-avatar"><PlayerAvatar src={opponent?.avatar}/></div><div className="mp-player-copy"><strong>{opponentName}</strong><small className="mp-player-level">⭐ {opponent?.level??1}</small><small><i className={`mp-online-dot ${opponent?.connected===false?"off":"busy"}`}/> {opponent?.connected===false?"Disconnected":"In match"}</small></div></div></header><section className="mp-board-section"><StakeDisplay roomCode={roomCode}/><div className="mp-board-frame"><CanonicalLudoBoard theme={cosmetics.board} yardSkin={cosmetics.yard} demoTokens={tokens} legalTokenKeys={legalTokenKeys} onTokenClick={chooseToken}/></div></section><section className="mp-control-section"><div className="mp-info-card"><div className="mp-info-name">{meName}</div><div className="mp-info-level">⭐ {mine?.level??profile.level}</div><div className="mp-info-coins">🪙 {(Number(mine?.coins)||profile.coins).toLocaleString()}</div></div><div className="mp-turn-card"><div className={`mp-turn-label ${myTurn?"mine":""}`}><span/>{statusText}</div><div className="mp-turn-note">{game?.status==="playing"?(myTurn?"Roll the dice to play":"Wait for the other player to move"):connectionMessage}</div><DemoDice value={roll} onRoll={handleRoll} disabled={diceDisabled} botRolling={remoteRolling} skin={cosmetics.dice}/></div><div className="mp-tools"><button type="button" className={`mp-tool ${chatOpen||chatUnread?"active":""}`} onClick={()=>setChatOpen(true)}><span>💬</span><small>Chat{chatUnread?" •":""}</small></button><div className="mp-tool"><ChatVoice roomCode={roomCode} playerId={me} members={commMembers} socket={socket}/><small>Mic</small></div></div></section><div className="mp-bottom-bar"><button type="button" className="danger" onClick={()=>setLeaveConfirmOpen(true)}>↩ Leave Match</button><button type="button" onClick={()=>setPlayersOpen(true)}>👥 Players</button><button type="button" onClick={toggleSound}>{soundEnabled?"🔊 Sound":"🔇 Sound"}</button><div className="mp-connection">● {connected?"Connected":"Reconnecting…"}</div><div className="mp-room">🛡️ Room ID: {roomCode}</div></div></div><ChatPanel open={chatOpen} messages={chatMessages} me={me} value={chatText} setValue={setChatText} onSend={sendChat} onClose={()=>setChatOpen(false)} onReaction={sendChat}/>{playersOpen&&<div className="mp-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setPlayersOpen(false);}}><section className="mp-players-modal" role="dialog" aria-modal="true"><div className="mp-chat-head"><strong>Players</strong><button type="button" onClick={()=>setPlayersOpen(false)} aria-label="Close players">×</button></div>{players.map(player=><div className="mp-roster-row" key={player.playerId}><span className="mp-roster-dot"/><strong>{player.name}{String(player.playerId)===String(me)?" (You)":""}</strong><small>{player.connected===false?"Disconnected":"Connected"}</small></div>)}</section></div>}<LudoConfirmModal open={leaveConfirmOpen} title="Leave match?" message="Leaving now will end your participation in this room." confirmLabel="Leave" cancelLabel="Stay" onConfirm={confirmLeave} onCancel={()=>setLeaveConfirmOpen(false)} danger/></main>;
}
