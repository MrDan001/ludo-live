...
const normalizeTokens=(serverTokens:TokenMap):DemoToken[]=>{
 ...
 Object.values(serverTokens||{}).forEach(pt=>Object.entries(pt||{}).forEach(([key,t])=>{if(!/^(red|yellow|green|blue):[0-3]$/.test(key))return;const p=Number(t?.position);if(Number.isFinite(p))positions[key as `${Color}:${number}`]=p;}));
 return COLORS.flatMap(color=>Array.from({length:4},(_,id)=>{const position=positions[`${color}:${id}`]??0;return {color,id,position,state:tokenState(position)};}));
};
const same=(a:DemoToken,b:DemoToken)=>a.color===b.color&&a.id===b.id;
...
export default function OnlineMultiplayerGame(){
 ...
 const [myLevel,setMyLevel]=useState(1); const [myCoins,setMyCoins]=useState(0);
 const [game,setGame]=useState<GameState|null>(null); const [roster,setRoster]=useState<Player[]>([]);
 const [tokens,setTokens]=useState<DemoToken[]>(()=>normalizeTokens({}));
 const [roll,setRoll]=useState<DiceValue>(6); const [pending,setPending]=useState<DiceValue|null>(null); const [remoteRolling,setRemoteRolling]=useState(false); const [animating,setAnimating]=useState(false);
 const [roomCode,setRoomCode]=useState("W100NB"); const [chatOpen,setChatOpen]=useState(false); const [playersOpen,setPlayersOpen]=useState(false); const [chatUnread,setChatUnread]=useState(false); const [chatText,setChatText]=useState("");
 ...
 const revisionRef=useRef(-1); const authoritativeRef=useRef<DemoToken[]>(tokens); const animatingRef=useRef(false); const moveQueueRef=useRef<MoveEvent[]>([]); const timersRef=useRef<number[]>([]); const mountedRef=useRef(true); const chatOpenRef=useRef(false);
 const requestPendingRef=useRef(false); const skipPendingRef=useRef(false);
 const setDisplay=(next:DemoToken[])=>{authoritativeRef.current=authoritativeRef.current; setTokens(next);};
 const clearTimers=()=>{timersRef.current.forEach(window.clearTimeout);timersRef.current=[];};
 const stateTokens=(s:GameState)=>normalizeTokens(s.tokens||{});
 const commitAuthoritativeIfIdle=useCallback(()=>{if(!animatingRef.current&&moveQueueRef.current.length===0)setTokens(authoritativeRef.current.map(t=>({...t}));},[]);
 const runAnimation=useCallback(async(move:MoveEvent)=>{
   if(!mountedRef.current)return;
   animatingRef.current=true; setAnimating(true); requestPendingRef.current=false; skipPendingRef.current=false;
   const key=String(move.tokenId); const [color,idText]=key.split(":"); const id=Number(idText); if(!COLORS.includes(color as Color)||!Number.isInteger(id)){animatingRef.current=false;setAnimating(false);return;}
   const current=authoritativeRef.current.map(t=>({...t})); const tokenIndex=current.findIndex(t=>t.color===color&&t.id===id); if(tokenIndex<0){animatingRef.current=false;setAnimating(false);return;}
   const update=(nextPosition:number)=>setTokens(prev=>prev.map(t=>t.color===color&&t.id===id?{...t,position:nextPosition,state:tokenState(nextPosition)}:t));
   const sleep=(ms:number)=>new Promise<void>(resolve=>{const timer=window.setTimeout(resolve,ms);timersRef.current.push(timer);});
   const from=Number(move.from); const contact=move.captureToCenter?Number(move.captureProgress??move.to):Number(move.to); const finalTo=Number(move.finalTo??(move.captureToCenter?FINISH_PROGRESS:move.to));
   update(from);
   if(from===0&&contact>0){ for(let p=1;p<=contact;p++){await sleep(280); if(!mountedRef.current)return; update(p);} }
   else if(contact!==from){ const step=contact>from?1:-1; for(let p=from+step;;p+=step){await sleep(280); if(!mountedRef.current)return; update(p); if(p===contact)break;} }
   if(move.captureToCenter&&move.captured){ setTokens(prev=>prev.map(t=>t.color===move.captured!.color&&t.id===move.captured!.id?{...t,position:0,state:"yard"}:t)); await sleep(220); if(!mountedRef.current)return; update(FINISH_PROGRESS); if(finalTo!==FINISH_PROGRESS){await sleep(140);update(finalTo);} }
   else { if(finalTo!==contact){await sleep(220);update(finalTo);} }
   await sleep(100);
   if(!mountedRef.current)return;
   const latest=authoritativeRef.current;
   setTokens(prev=>prev.map(t=>{const latestToken=latest.find(x=>same(x,t)); return latestToken&&!same(t,{...latestToken,position:t.position,state:t.state} as DemoToken)?t:t;}));
   const q=moveQueueRef.current.shift();
   if(q){ await runAnimation(q); return; }
   animatingRef.current=false; setAnimating(false); setTokens(authoritativeRef.current.map(t=>({...t})));
 },[]);
 const enqueueMove=useCallback((move:MoveEvent)=>{moveQueueRef.current.push(move); if(!animatingRef.current){const next=moveQueueRef.current.shift(); if(next)void runAnimation(next);}},[runAnimation]);

 useEffect(()=>{chatOpenRef.current=chatOpen;},[chatOpen]);
 useEffect(()=>{mountedRef.current=true; return()=>{mountedRef.current=false; clearTimers();};},[]);
 ...
 const players=useMemo(()=>{
   const byId=new Map((game?.players||[]).map(p=>[String(p.playerId),p]));
   if(roster.length)return roster.map(m=>({...byId.get(String(m.playerId)),...m} as Player));
   return game?.players||[];
 },[game?.players,roster]);
 ...
 useEffect(()=>{
   ...
   const connect=async()=>{
     ...
     try{const [a,c]=await Promise.all([fetch("/api/auth",{cache:"no-store"}),fetch("/api/customization",{cache:"no-store"})]);const d=await a.json();const customization=c.ok?await c.json():null;playerId=String(d?.user?.id||"");profileName=String(d?.user?.username||"");profileAvatar=String(d?.user?.avatar||d?.user?.image||"");profileLevel=Math.max(1,Number(d?.user?.level)||1);profileCoins=Math.max(0,Number(d?.user?.coins)||0);const eq=String(customization?.equippedAvatar||"");const found=Array.isArray(customization?.avatars)?customization.avatars.find((x:{id?:string})=>x.id===eq):null;if(found?.imageUrl||found?.icon)profileAvatar=String(found.imageUrl||found.icon);}catch{}
     if(!mounted)return; setMe(playerId);setMyName(profileName);setMyAvatar(profileAvatar);setMyLevel(profileLevel);setMyCoins(profileCoins);
     const room=params.get("room")||roomCode; const roomSize=Number(params.get("size")||2);
     s=io(window.location.origin,{transports:["websocket","polling"],reconnection:true}); setSocket(s);
     s.on("connect",()=>{if(room&&playerId)s?.emit("join-room",{roomCode:room,name:profileName,avatar:profileAvatar,level:profileLevel,coins:profileCoins,roomSize,playerId});});
     ...
     s.on("game-dice",(e:{value:DiceValue})=>{setRoll(e.value);setRemoteRolling(true);const t=window.setTimeout(()=>setRemoteRolling(false),900);timersRef.current.push(t);});
     s.on("chat",(m:{id?:string;name?:string;text?:string;at?:number;playerId?:string})=>{if(!mounted||!m?.text)return;const id=String(m.id||""),name=String(m.name||"Player"),text=String(m.text),at=Number(m.at||Date.now());setChatMessages(items=>id&&items.some(x=>x.id===id)?items:[...items.slice(-99),{id:id||`remote-${at}`,name,text,at}]);if(!chatOpenRef.current&&String(m.playerId||"")!==playerId&&name!==profileName)setChatUnread(true);});
     s.on("game-state",(next:GameState)=>{if(!mounted)return;const r=Number(next.stateRevision??-1);if(r>=0&&revisionRef.current>=0&&r<revisionRef.current)return;if(r>=0)revisionRef.current=r;setGame(next);setPending(String(next.currentPlayerId||"")===playerId?next.pendingMove:null);authoritativeRef.current=stateTokens(next);if(!animatingRef.current&&moveQueueRef.current.length===0)setTokens(authoritativeRef.current.map(t=>({...t})));});
     ...
     s.on("game-move-error",()=>{requestPendingRef.current=false;skipPendingRef.current=false;setAnimating(false);});
   };
   void connect(); return()=>{mounted=false;s?.disconnect();};
 },[params,roomCode,enqueueMove]);

 useEffect(()=>{if(!socket||!game||!myTurn||pending===null||animating||requestPendingRef.current||skipPendingRef.current)return;if(!tokens.some(t=>myColors.includes(t.color)&&canMove(tokens,t,pending))){skipPendingRef.current=true;socket.emit("game-move",{tokenId:"__skip__"});}},[socket,game,myTurn,pending,animating,tokens,myColors]);
 const chooseToken=useCallback((color:Color,id:number)=>{if(!socket||!game||!myTurn||pending===null||animating||requestPendingRef.current)return;const token=tokens.find(t=>t.color===color&&t.id===id);if(!token||!myColors.includes(color)||!canMove(tokens,token,pending))return;requestPendingRef.current=true;socket.emit("game-move",{tokenId:`${color}:${id}`});},[socket,game,myTurn,pending,animating,tokens,myColors]);
 const handleRoll=useCallback(()=>{if(!socket||!game||!myTurn||pending!==null||animating||remoteRolling)return;socket.emit("game-roll");},[socket,game,myTurn,pending,animating,remoteRolling]);
