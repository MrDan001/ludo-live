...
export default function OnlineMultiplayerGame(){
  ...
  useEffect(()=>{
    ...
    const start=async()=>{
      ...
      s.on("game-dice",(event:{value:DiceValue;playerId?:string})=>{if(Number(event?.value)>=1)setRoll(event.value);if(String(event?.playerId||"")!==String(p.playerId)){setRemoteRolling(true);window.setTimeout(()=>mountedRef.current&&setRemoteRolling(false),420);}else{setRemoteRolling(false);clearRollRequest();}});
