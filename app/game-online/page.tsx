...
function GameContent() {
  ...
  }, [game?.players, roster, me, myName, myAvatarUrl, myLevel, myCoins]);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      try {
        ...
        playerId = String(d?.user?.id || ""); profileName = String(d?.user?.username || ""); profileAvatar = String(d?.user?.avatar || d?.user?.image || "");
        profileLevel = Math.max(1, Number(d?.user?.level) || 1); profileCoins = Math.max(0, Number(d?.user?.coins) || 0);
      ...
      localSocket.on("roster", (members: Player[]) => { const nextRoster = Array.isArray(members) ? members : []; setRoster(nextRoster); setGame((g) => g ? { ...g, players: nextRoster } : { status: "waiting", currentPlayerId: null, dice: null, pendingMove: null, players: nextRoster, tokens: {} }); });
      ...
      localSocket.on("chat", (message: { id?: string; name?: string; text?: string; at?: number; playerId?: string }) => {
  ...
  return <main className="ll-rebuilt-page"><div className="ll-rebuilt-shell">
    <header className="ll-topbar">
      <div className="ll-player-card ll-player-you"><div className="ll-top-avatar"><PlayerAvatar src={mine?.avatar} fallback="👤" /></div><div className="ll-player-copy"><div className="ll-player-name"><strong>{mine?.name || myName || "Player"}</strong><span>YOU</span></div><div className="ll-player-status"><i />{myTurn ? "Your Turn" : "Waiting"}</div></div><b className="ll-level">★ {mine?.level || myLevel}</b></div>
      <div className="ll-logo"><span>♛</span><strong>LUDO</strong><small>LIVE</small></div>
      <div className="ll-player-card ll-player-opponent"><div className="ll-top-avatar"><PlayerAvatar src={opponent?.avatar} fallback="👤" /></div><div className="ll-player-copy"><div className="ll-player-name"><strong>{opponent?.name || ""}</strong></div><div className={`ll-player-status ${opponent ? "opponent-in" : "waiting"}`}><i />{opponent ? "IN MATCH" : "WAITING"}</div></div>{opponent && <b className="ll-level">★ {opponent.level || 1}</b>}</div>
    </header>
    <section className="ll-board-stage"><div className="ll-board-frame"><CanonicalLudoBoard theme={theme} demoTokens={tokens} onTokenClick={chooseToken} legalTokenKeys={legalTokenKeys} /></div></section>
    <section className="ll-hud"><div className="ll-main-controls">
      <div className="ll-profile-card"><div className="ll-profile-head"><div className="ll-profile-avatar"><PlayerAvatar src={mine?.avatar} fallback="👤" /></div><div className="ll-profile-text"><strong>{mine?.name || myName || "Player"}</strong><span>★ {mine?.level || myLevel}</span></div><em>✎</em></div><div className="ll-coins"><span>🟡</span><b>{(mine?.coins ?? myCoins).toLocaleString()}</b></div></div>
      <div className="ll-dice-card"><div className="ll-turn-copy"><strong><i />{myTurn ? "YOUR TURN" : "OPPONENT'S TURN"}</strong><span>{myTurn ? <>Roll the dice and<br />make your move</> : <>Wait for the other<br />player to move</>}</span></div><div className="ll-dice-slot"><DemoDice value={roll} onRoll={handleRoll} disabled={!myTurn || pending !== null || animating || remoteRolling} botRolling={remoteRolling} /></div></div>
      <div className="ll-action-stack"><button type="button" onClick={() => setChatOpen((v) => { const next = !v; if (next) setChatUnread(false); return next; })}><b>•••</b><span>Chat</span>{chatUnread && <i aria-label="New chat message" style={{ position:"absolute",top:6,right:6,width:8,height:8,borderRadius:"50%",background:"#ff2941",boxShadow:"0 0 8px #ff2941" }} />}</button><div className="ll-mic"><ChatVoice socket={socket} roomCode={roomCode} playerId={me} members={players.map((p) => ({ id: p.playerId, playerId: p.playerId, name: p.name, online: true, peerId: p.peerId }))} /></div></div>
    </div><div className="ll-reactions">{QUICK_REACTIONS.map((text, i) => <button type="button" key={i} onClick={() => sendQuickReaction(text)}>{text}</button>)}</div><footer className="ll-footer"><button type="button" className="leave" onClick={leaveRoom}>↪ Leave Match</button><button type="button" onClick={() => setPlayersOpen((v) => !v)}>👥 Players</button><button type="button" onClick={toggleSound}>{soundEnabled ? "🔊 Sound" : "🔇 Sound"}</button><div className="ll-room">🛡️ <span>Room ID: {roomCode}</span> ▢</div></footer></section>
...
export default function OnlineGamePage() { return <Suspense fallback={<div style={{ background: "#000", height: "100vh" }} />}><GameContent /></Suspense>; }