...
function SocialContent(){
 ...
 const [name,setName]=useState("PlayerOne");const [rooms,setRooms]=useState<ChatRoom[]>([]);const [room,setRoom]=useState<ChatRoom|null>(null);const [members,setMembers]=useState<Member[]>([]);const [messages,setMessages]=useState<Message[]>([]);const [text,setText]=useState("");const [title,setTitle]=useState("");const [code,setCode]=useState("");const [notice,setNotice]=useState("");const [connected,setConnected]=useState(false);const [socket,setSocket]=useState<Socket|null>(null);const pendingRef=useRef<"create"|"join"|null>(null);const gate=useAccountGate();
 useEffect(()=>{const s=io(window.location.origin,{transports:["websocket","polling"]});setSocket(s);s.on("connect",()=>{setConnected(true);s.emit("list-chat-rooms")});s.on("disconnect",()=>setConnected(false));s.on("chat-room-list",setRooms);s.on("chat-room-joined",(payload)=>{if(pendingRef.current==="create")bumpStat("chatRoomsCreated");if(pendingRef.current==="join")bumpStat("chatRoomsJoined");pendingRef.current=null;setRoom({code:payload.code,title:payload.title,hostName:name,members:payload.members.length,maxMembers:20,locked:false});setMembers(payload.members);setMessages(payload.messages||[]);setNotice("")});s.on("chat-room-members",(m:Member[])=>{setMembers(m);setRoom(r=>r?{...r,members:m.length}:r)});s.on("chat-room-message",(m:Message)=>{setMessages(x=>[...x,m].slice(-100));if(m.id===s.id)bumpStat("messagesSent")});s.on("chat-room-error",(m:string)=>{pendingRef.current=null;setNotice(m)});s.on("chat-kicked",()=>{setRoom(null);setMembers([]);setMessages([]);setNotice("You were removed from that chat room by its host.")});return()=>{s.disconnect()}},[]);
 const create=()=>gate.check(()=>{if(!socket||!connected)return;pendingRef.current="create";socket.emit("create-chat-room",{title:title.trim()||`${name}'s Chat`,name});setTitle("")});
 const join=(roomCode:string)=>gate.check(()=>{if(!socket)return;pendingRef.current="join";socket.emit("join-chat-room",{roomCode,name})});
 ...
 return <AppFrame back="/home"><div style={{maxWidth:980,margin:"0 auto",paddingBottom:50}}>
  ...
  <div style={tabs}><button onClick={()=>setTab("chat")} style={{...tabStyle,background:tab==="chat"?"#2563eb":"transparent",color:tab==="chat"?"#fff":"#94a3b8"}}>Chat Rooms</button><button onClick={()=>setTab("friends")} style={{...tabStyle,background:tab==="friends"?"#2563eb":"transparent",color:tab==="friends"?"#fff":"#94a3b8"}}>Friends</button></div>
  {tab==="friends"?<section style={panel}><h2>👥 Find players</h2><p style={muted}>Friend connections are available to registered players. Guests can browse the app but cannot send friend requests or connect with other players.</p><button onClick={()=>gate.check(()=>setNotice("Your account can use friend connections."))} style={primary}>＋ SEND FRIEND REQUEST</button>{notice&&<div style={{marginTop:12,padding:12,borderRadius:12,background:"#082c18",color:"#86efac"}}>{notice}</div>}</section>:<>
...
const tabStyle={border:0,borderRadius:10,padding:"10px 15px",fontWeight:900,cursor:"pointer"};
