...
function SocialContent(){
 ...
 return <AppFrame back="/home"><div style={{maxWidth:980,margin:"0 auto"}}>
  ...
  {tab==="missions"&&<Section title="Missions"><div style={list}>{[["Win 3 matches","2 / 3","1,000 🪙"],["Play 5 matches","5 / 5","1,000 🪙"],["Win a match with 4 players","1 / 1","1,000 🪙"],["Collect 500 coins","200 / 500","1,000 🪙"],["Use spin wheel 2 times","1 / 2","500 🪙"]].map(([t,p,r])=><Card key={t}><span style={{fontSize:22}}>{p===t?"❌":"✅"}</span><div style={{flex:1}}><b>{t}</b><div style={{height:7,background:"#172554",borderRadius:9,marginTop:7}}><div style={{height:"100%",width:p.startsWith("5")||p.startsWith("1 / 1")?"100%":"55%",background:"#22c55e",borderRadius:9}}/></div><small style={{color:"#94a3b8"}}>{p}</small></div><button style={smallGreen} onClick={()=>claim(t)}>{claimed.includes(t)?"Claimed":"Claim"}</button></Card>)}</div></Section>}
...
function Section({title,children}:{title:string;children:React.ReactNode}){return <section style={{marginTop:18,padding:18,borderRadius:20,background:"linear-gradient(145deg,#071b3c,#050d20)",border:"1px solid rgba(96,165,250,.18)"}}><h2 style={{marginTop:0}}>{title}</h2>{children}</section>}
function Card({children}:{children:React.ReactNode}){return <div style={{display:"flex",alignItems:"center",gap:12,padding:12,borderRadius:14,background:"rgba(15,31,62,.78)",border:"1px solid rgba(96,165,250,.12)"}}>{children}</div>}
