...
const meProfile=():Profile=>({id:pid(),name:pname().slice(0,24),level:Number(localStorage.getItem("ludo-level")||0),rank:localStorage.getItem("ludo-rank")||"Bronze",badges:[],streak:Number(localStorage.getItem("ludo-streak")||0)});

const mini={border:0,borderRadius:8,padding:"8px 12px",marginLeft:4,marginBottom:4,minWidth:72,background:"#2563eb",color:"#fff",fontWeight:800,fontSize:11};
...
export default function Chat(){
 ...
 useEffect(()=>{
  if(!getAccount()){window.location.replace(`/account?next=${encodeURIComponent("/chat")}`);return}
 ...
 const create=()=>{if(!getAccount()){window.location.href="/account?next=/chat";return}socket?.emit("create-chat-room",{title:title||`${name}'s Chat`,playerId});setTitle("");setCreateOpen(false)};
 const join=(code:string)=>{if(!getAccount()){window.location.href="/account?next=/chat";return}socket?.emit("join-chat-room",{roomCode:code,playerId});};
 const send=()=>{if(text.trim()&&socket&&room){socket.emit("chat-room-message",{text:text.trim()});setText("")}};
 const addFriend=(id:string)=>{if(!getAccount()){window.location.href="/account?next=/chat";return}socket?.emit("send-friend-request",{targetId:id})};
 ...
 if(!accountReady)return <main className="subpage" style={{display:"grid",placeItems:"center",minHeight:"70vh"}}><div className="empty-big"><h2>Account required</h2><p>Creating or joining chat rooms and using friend features requires a player account.</p></div></main>;
 ...
 return <main className="subpage">
  ...
  {tab==="friends"&&<section className="room-browser chat-browser">
   ...
   {profile&&<div style={{...panel,top:12,bottom:"auto"}}><button onClick={()=>setProfile(null)} style={close}>×</button><h2>{profile.name}</h2><p>Rank: <b>{profile.rank}</b></p><p>Level: <b>{profile.level}</b></p><p>🏅 {profile.badges.length?profile.badges.join(" · "):"No badges yet"}</p><p>🔥 Streak: <b>{profile.streak}</b></p></div>}
  </section>}
