...
export default function HomePage() {
  ...
  const page: React.CSSProperties = {
    position: "fixed", inset: 0, width: "100%", height: "100svh", overflow: "hidden",
    background: "linear-gradient(180deg,#031536 0%,#020b1d 52%,#010611 100%)",
    color: "#fff", fontFamily: "Arial,Helvetica,sans-serif"
  };
  const shell: React.CSSProperties = {
    width: "100%", maxWidth: 720, height: "calc(100svh - 68px - env(safe-area-inset-bottom, 0px))",
    margin: "0 auto", padding: "6px 12px", boxSizing: "border-box", display: "grid",
    gridTemplateRows: "auto minmax(0,1fr) 88px 76px", gap: 7, overflow: "hidden"
  };
  ...
  const friendsStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", alignItems: "center", gap: 8, padding: "9px 11px 9px 13px", borderRadius: 18, background: "#0a214b", border: "1px solid #173766", minWidth: 0 };
    ...
    <div style={shell}>
      <header style={header}>
        <Link href="/profile" style={profile}>
          <div style={avatarWrap}><div style={avatar}><EquippedAvatar /></div><div style={badge}>{progress.level}</div></div>
      ...
      <section style={actionsStyle}>{actions.map(a => <Link key={a.title} href={a.href} style={{ minHeight: 0, display: "grid", gridTemplateColumns: "clamp(54px,12vw,82px) minmax(0,1fr) 20px", alignItems: "center", gap: 10, padding: "5px 12px", borderRadius: 18, border: "1px solid rgba(255,255,255,.25)", color: "#fff", textDecoration: "none", background: a.bg, boxShadow: "0 4px 12px rgba(0,0,0,.18)" }}><span style={{ textAlign: "center", fontSize: "clamp(32px,7vw,52px)", lineHeight: 1 }}>{a.icon}</span><span style={{ minWidth: 0 }}><b style={{ display: "block", fontSize: "clamp(15px,3.6vw,25px)", lineHeight: 1.05, fontWeight: 950 }}>{a.title}</b><small style={{ display: "block", marginTop: 4, fontSize: "clamp(9px,2vw,14px)", lineHeight: 1.15, fontWeight: 750 }}>{a.sub}</small></span><span style={{ fontSize: 30, textAlign: "center" }}>›</span></Link>)}</section>
      ...
      <Link href="/friends" style={{ ...friendsStyle, color: "inherit", textDecoration: "none" }}><div style={{ minWidth: 0 }}><b style={{ display: "block", color: "#66b7ff", fontSize: "clamp(12px,3vw,16px)" }}>👥 FRIENDS</b><span style={{ display: "block", marginTop: 3, color: "#a9bddb", fontSize: "clamp(9px,2.5vw,13px)", whiteSpace: "nowrap" }}>Find friends and organize your next match.</span></div><span style={{ border: 0, borderRadius: 12, padding: "11px clamp(13px,3.5vw,20px)", background: "#37b92e", color: "#fff", fontWeight: 950, fontSize: "clamp(10px,2.7vw,14px)" }}>OPEN</span></Link>
    </div>
