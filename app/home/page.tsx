  ...
  return <main style={{ minHeight: "100vh", background: "linear-gradient(180deg,#031536 0%,#020b1d 48%,#010611 100%)", color: "#fff", paddingBottom: 92 }}>
    <div style={{ width: "100%", maxWidth: 560, margin: "0 auto", padding: "12px 14px 0", boxSizing: "border-box" }}>
      <header style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12, alignItems: "center", padding: "8px 2px 14px" }}>
        <Link href="/profile" style={{ color: "#fff", textDecoration: "none", display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
      ...
      <section style={{ marginTop: 14, padding: 16, borderRadius: 18, background: "linear-gradient(135deg,#071c45,#0a1630)", border: "1px solid rgba(59,130,246,.24)" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}><div><strong style={{ fontSize: 16 }}>🟢 LIVE ROOMS</strong><div style={{ color: "#94a3b8", fontSize: 12, marginTop: 4 }}>Players create rooms here. Join one that still has space.</div></div><Link href="/lobby" style={{ color: "#60a5fa", textDecoration: "none", fontWeight: 900, fontSize: 12 }}>OPEN LOBBY ›</Link></div><div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><Link href="/room?action=create&size=4" style={roomButton}>＋ CREATE ROOM</Link><Link href="/lobby" style={roomButtonAlt}>👥 FIND PLAYERS</Link></div></section>
    </div>
    <nav style={{ position: "fixed", zIndex: 20, left: 0, right: 0, bottom: 0, maxWidth: 560, margin: "0 auto", height: 72, display: "grid", gridTemplateColumns: "repeat(4,1fr)", background: "rgba(2,12,31,.98)", borderTop: "1px solid rgba(72,111,175,.3)" }}><Link href="/home" style={bottomStyle(true)}>⌂<small>Home</small></Link><Link href="/social?tab=friends" style={bottomStyle(false)}>👥<small>Friends</small></Link><Link href="/chat" style={bottomStyle(false)}>💬<small>Chat</small></Link><Link href="/profile" style={bottomStyle(false)}>👤<small>Profile</small></Link></nav>
  </main>;
}
...
const roomButton={display:"flex",alignItems:"center",justifyContent:"center",padding:"12px 8px",borderRadius:11,background:"#16a34a",color:"#fff",textDecoration:"none",fontWeight:950,fontSize:12};
const roomButtonAlt={display:"flex",alignItems:"center",justifyContent:"center",padding:"12px 8px",borderRadius:11,background:"#12346c",border:"1px solid #2b5ba8",color:"#fff",textDecoration:"none",fontWeight:950,fontSize:12};
