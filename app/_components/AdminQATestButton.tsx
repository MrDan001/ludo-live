...
export default function AdminQATestButton() {
  ...
  const boardPage = /(^|\/)(board|game|game-online|tournament)(\/|$)/.test(pathname || "");

  useEffect(() => { if (!boardPage) return; fetch("/api/admin", { cache: "no-store" }).then(r => setAdmin(r.ok)).catch(() => setAdmin(false)); }, [boardPage]);
  if (!boardPage || !admin) return null;

  async function runTests() {
    setBusy(true); setResult(null);
    try { const r = await fetch("/api/qa", { cache: "no-store" }); setResult(await r.json()); }
    catch { setResult({ error: "Could not run QA tests." }); }
    finally { setBusy(false); }
  }

  return <>
    <button type="button" onClick={() => setOpen(v => !v)} style={{position:"fixed",right:12,bottom:12,zIndex:9999,border:0,borderRadius:999,padding:"10px 14px",background:"#111827",color:"#fff",fontWeight:800,boxShadow:"0 8px 30px rgba(0,0,0,.3)"}}>🧪 QA TEST</button>
