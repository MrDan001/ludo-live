...
export default function CanonicalLudoBoard({theme="classic",demoTokens=[],onTokenClick}:Props){
 ...
 const accent=BOARD_PALETTES[theme]?.accent||BOARD_PALETTES.classic.accent;
 return <div className="canonical-ludo-frame" style={{position:"relative",width:"100%",aspectRatio:"1",borderRadius:17,"--board-accent":accent} as React.CSSProperties} aria-label={`${BOARD_NAMES[theme]} canonical Ludo board`}>
  <style>{`
    ...
    .canonical-ludo-frame::before,.canonical-ludo-frame::after{content:"";position:absolute;pointer-events:none;border-radius:inherit;box-sizing:border-box}
    .canonical-ludo-frame::before{inset:-9px;border:1.5px solid var(--board-accent);opacity:.72;box-shadow:0 0 0 1px color-mix(in srgb,var(--board-accent) 24%,transparent),0 8px 24px color-mix(in srgb,var(--board-accent) 20%,transparent);z-index:0}
    .canonical-ludo-frame::after{inset:-4px;border:1px solid color-mix(in srgb,var(--board-accent) 44%,transparent);box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--board-accent) 12%,transparent);z-index:0}
  `}</style>
