...
const BOARD_BORDER_PX=3;
const cellCenter=(index:number)=>`${((index+.5)*100/BOARD_GRID_SIZE).toFixed(6)}%`;
const cellCenterWithBorder=(index:number,axis:"x"|"y")=>{
 const center=((index+.5)/BOARD_GRID_SIZE)*100;
 const borderCorrection=BOARD_BORDER_PX*(1-2*(index+.5)/BOARD_GRID_SIZE);
 return `calc(${center.toFixed(6)}% + ${borderCorrection.toFixed(3)}px)`;
};
export default function CanonicalLudoBoard({theme="classic",demoTokens=[],onTokenClick}:Props){
 ...
 return <div style={{position:"relative",width:"100%",aspectRatio:"1"}} aria-label={`${BOARD_NAMES[theme]} canonical Ludo board`}>
  <LudoBoard theme={theme} demoTokens={demoTokens.filter(t=>t.state==="yard")} onTokenClick={onTokenClick} style={{width:"100%",height:"100%"}}/>
  {moving.map(t=><button key={`${t.color}-${t.id}`} type="button" onClick={()=>onTokenClick?.(t.color,t.id)} aria-label={`${t.color} token`} style={{position:"absolute",left:cellCenterWithBorder(t.col,"x"),top:cellCenterWithBorder(t.row,"y"),transform:"translate(-50%,-50%)",width:"5.1%",aspectRatio:1,border:"2px solid #222",background:BOARD_PALETTES[theme][t.color],zIndex:30,fontWeight:900,padding:0,color:"transparent",fontSize:0}} />)}
