    ...
    <div style={overlay} onClick={close}>
      <div onClick={e => e.stopPropagation()} style={{ ...card, borderColor: `${accent}55` }}>
        ...
        {isReview ? <>
          ...
          <button disabled={busy} onClick={() => { setReason(""); setDialog({ type: "deny", player }); }} style={{ width: "100%", marginTop: 9, padding: 14, border: "1px solid rgba(248,113,113,.3)", borderRadius: 15, background: "rgba(239,68,68,.12)", color: "#fca5a5", fontWeight: 900 }}>✕ Deny Review</button>
        </> : isSuspend ? <>
