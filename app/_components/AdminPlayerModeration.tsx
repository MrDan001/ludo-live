...
export default function AdminPlayerModeration() {
  ...
  useEffect(() => {
    if (pathname !== "/dbase/players" || !players.length) return;
    const rows = Array.from(document.querySelectorAll<HTMLElement>(".case-row"));
    rows.forEach(row => {
      if (row.querySelector("[data-player-moderation]")) return;
      ...
      bar.style.cssText = "display:flex;gap:7px;flex-wrap:wrap;align-items:center;margin-top:10px;width:100%;";
      const add = (label: string, kind: string, danger = false) => {
        const b = document.createElement("button");
        b.textContent = label;
        b.className = "admin-btn";
        if (danger) b.classList.add("danger-btn");
        b.onclick = () => {
          if (kind === "message") { setSelected(player); setMessage(""); setTitle("Message from Ludo Live"); setReason(""); }
          if (kind === "suspend") { setSelected(player); setReason(""); setMessage(""); setTitle("suspend"); }
  ...
  const moderate = async (action: string, player: Player, extra: Record<string, any> = {}) => {
    ...
    try {
      ...
      await load();
      setSelected(null);
      alert(action === "message" ? "Message sent." : action === "suspend" ? "Player suspended." : action === "unsuspend" ? "Player unsuspended." : "Player deleted.");
  ...
  const deletePlayer = async (player: Player) => {
    ...
    const ok = window.confirm(`Delete ${name} permanently? This removes the player from the server and cannot be undone.`);
    if (!ok) return;
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 1000000, background: "rgba(0,0,0,.68)", display: "grid", placeItems: "center", padding: 20 }} onClick={close}>
    <div onClick={e => e.stopPropagation()} style={{ width: "min(520px,100%)", background: "#0a1729", color: "#fff", border: "1px solid rgba(255,255,255,.12)", borderRadius: 24, padding: 24, boxShadow: "0 30px 100px rgba(0,0,0,.6)" }}>
      <div style={{ fontSize: 12, opacity: .6, fontWeight: 800, letterSpacing: 1.5 }}>{isSuspend ? "SUSPEND PLAYER" : "DIRECT PLAYER MESSAGE"}</div>
