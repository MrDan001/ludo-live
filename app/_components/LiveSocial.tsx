...
const atlasAvatarStyle = (id?: string): React.CSSProperties | null => {
  const match = id?.match(/^(premium|elite)-(\d{2})$/);
  if (!match) return null;
  const n = match[1] === "premium" ? Number(match[2]) : 10 + Number(match[2]);
  const i = n - 1;
  return {
    display: "block",
    width: "100%",
...
export default function LiveSocial({ roomCode, name, host = false, roomSize = 4, stake = 0, compact = false, onStart, onKicked, leaveRequested = false, onLeaveComplete }: { roomCode: string; name: string; host?: boolean; roomSize?: number; stake?: number; compact?: boolean; onStart?: () => void; onKicked?: () => void; leaveRequested?: boolean; onLeaveComplete?: () => void }) {
  ...
  return <section className={`live-social${compact ? " live-social-compact" : ""}`}>
    ...
    <div className="room-stage">
      ...
      <div className="room-controls"><div className="room-voice"><ChatVoice roomCode={roomCode} playerId={selfPlayerId} members={voiceMembers}/></div><button type="button" onClick={toggleReady} className={`room-ready${ready ? " is-ready" : ""}`}>{ready ? "✓ Ready" : "Ready"}</button><button type="button" onClick={announceStart} disabled={!canStart} className="room-start">{bet.allStaked ? "START GAME" : "STAKE TO START"}</button><button type="button" onClick={() => setChatOpen(true)} className="room-chat-jump">💬</button></div>
      {notice && <div className="live-social-notice">{notice}</div>}
    </div>
    {chatOpen && <div className="room-chat-drawer" onClick={() => setChatOpen(false)}><div className="room-chat-sheet" onClick={e => e.stopPropagation()}><div className="room-chat-top"><b>Room chat</b><button type="button" className="room-chat-close" onClick={() => setChatOpen(false)}>Close</button></div><div className="room-chat-messages">{messages.length === 0 ? <span className="room-chat-empty">No messages yet. Agree the stake here before the host sets it.</span> : messages.map((m, i) => <div key={m.id + "-" + i} className="room-chat-message"><b>{m.name}:</b> {m.text}</div>)}</div><div className="room-chat-quick">{QUICK.map(q => <button key={q} type="button" onClick={() => send(q)}>{q}</button>)}</div><div className="room-chat-composer"><input className="room-chat-input" value={text} onChange={e => setText(e.target.value)} onKeyDown={e => { if (e.key === "Enter") send(); }} placeholder="Talk about the stake…"/><button type="button" className="room-chat-send" onClick={() => send()}>Send</button></div></div></div>}
  </section>;
}
