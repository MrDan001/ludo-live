...
declare global { interface Window { Peer?: any; webkitAudioContext?: typeof AudioContext } }
...
export default function ChatVoice({ roomCode, playerId, members }: { roomCode: string; playerId: string; members: Member[] }) {
  ...
  const setAvatarSpeaking = (pid: string, on: boolean) => {
    ...
    if (!name) return;
    document.querySelectorAll<HTMLButtonElement>(".ig-avatar").forEach((button) => {
      if (button.getAttribute("aria-label") === name) button.classList.toggle("voice-speaking", on);
    });
  };
  const broadcastSpeaking = (on: boolean) => {
    Object.values(connections.current).forEach((connection: any) => {
      const data = connection?.data;
      if (data?.open) { try { data.send({ type: "speaking", playerId: String(playerId), speaking: on }); } catch {} }
    });
  };
  ...
  useEffect(() => {
    members.forEach((member) => {
      ...
      if (pid && pid !== String(playerId)) connectPeer(`ludo-chat-${safe(roomCode)}-${safe(pid)}`, member.name || "Player", pid);
    });
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        peer.on("call", (call: any) => {
          ...
          call.on("stream", (stream: MediaStream) => { delete pendingCalls.current[call.peer]; playRemote(call.peer, stream); });
          call.on("close", () => { delete pendingCalls.current[call.peer]; removePeer(call.peer); });
          call.on("error", () => {});
        });
    ...
    return () => {
      ...
      Object.values(connections.current).forEach((connection: any) => { try { connection.data?.close?.(); connection.voiceCall?.close?.(); } catch {} });
      connections.current = {};
      ...
      document.querySelectorAll(".ig-avatar.voice-speaking").forEach((element) => element.classList.remove("voice-speaking"));
    };
  ...
  const toggleMic = async () => {
    ...
    try {
      if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) { setNotice("Microphone requires a secure HTTPS connection."); return; }
      if (!peerRef.current || !ready) { setNotice("Voice is still connecting. Try again in a moment."); return; }
  ...
  return (
    <div className="voice-control">
      <button type="button" onClick={() => setPanelOpen((value) => !value)} aria-expanded={panelOpen} className={`voice-button ${mic ? "active" : ""} ${!ready ? "connecting" : ""}`} title="Voice controls">
