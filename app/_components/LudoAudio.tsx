  ...
  useEffect(() => {
    const play = (kind: SoundKind) => {
      try {
        ...
        if(kind==="dice"){
          // A short wooden/plastic rattle: distinct, present, but not a harsh tone.
          tone(190,.11,0,.24,"triangle",120);
          tone(285,.10,.07,.22,"triangle",170);
          tone(160,.12,.14,.20,"triangle",105);
          tone(330,.10,.22,.18,"triangle",190);
          tone(230,.12,.29,.16,"triangle",130);
        }else if(kind==="move"){
        ...
        const end=kind==="win"?1.05:.75;
        master.gain.setValueAtTime(0.0001,now); master.gain.linearRampToValueAtTime(1,now+.006); master.gain.setValueAtTime(1,now+.02); master.gain.linearRampToValueAtTime(.0001,now+end);
      }catch{}
    };
