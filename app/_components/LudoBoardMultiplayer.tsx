...
const CENTER_SLOTS: Record<DemoToken["color"], Array<[number,number]>> = { red:[[4.35,4.35],[4.35,9.65],[9.65,4.35],[9.65,9.65]], green:[[4.35,5.35],[4.35,10.65],[9.65,5.35],[9.65,10.65]], yellow:[[5.35,4.35],[5.35,9.65],[10.65,4.35],[10.65,9.65]], blue:[[5.35,5.35],[5.35,10.65],[10.65,5.35],[10.65,10.65]] };
...
export default function LudoBoardMultiplayer({theme="classic",preview=false,className="",style,demoTokens=[],onTokenClick,snapOnUpdate=false,finishSound=false,animateUpdates=true,legalTokenKeys=[]}:Props){
 ...
 const[displayTokens,setDisplayTokens]=useState<DemoToken[]>(tokens);const displayRef=useRef(tokens);const timersRef=useRef<Record<string,number>>({});const launchTimersRef=useRef<Record<string,number>>({});const[launchingKeys,setLaunchingKeys]=useState<Set<string>>(new Set());const mountedRef=useRef(false);const receivedServerStateRef=useRef(false);
 ...
 </div><style jsx global>{`@keyframes mpTokenBreath{0%,100%{opacity:.42;transform:scale(.94)}50%{opacity:.95;transform:scale(1.05)}}`}</style></div>;
}
