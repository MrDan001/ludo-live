...
export default function LudoBoardMultiplayer({theme="classic",preview=false,className="",style,demoTokens=[],onTokenClick,snapOnUpdate=false,finishSound=false,animateUpdates=true,legalTokenKeys=[]}:Props){
 ...
 useEffect(()=>{
  ...
  for(const[key,target]of incoming){const currentToken=current.get(key);if(!currentToken){current.set(key,target);continue;}const from=Number(currentToken.position),to=Number(target.position);if(from===to||timersRef.current[key])continue;
   ...
   if(from===0&&to===1&&target.state==="track"){
    const launched=displayRef.current.map(t=>keyOf(t)===key?{...target,position:1,state:"track" as const}:t);displayRef.current=launched;setDisplayTokens(launched);setLaunchingKeys(p=>new Set(p).add(key));
    if(launchTimersRef.current[key])window.clearTimeout(launchTimersRef.current[key]);launchTimersRef.current[key]=window.setTimeout(()=>{setLaunchingKeys(p=>{const n=new Set(p);n.delete(key);return n});delete launchTimersRef.current[key]},220);emitAudio("move");delete timersRef.current[key];continue;
   }
   const direction=to>from?1:-1;
   const advance=()=>{const live=displayRef.current.find(t=>keyOf(t)===key);if(!live){delete timersRef.current[key];return;}const previous=Number(live.position),next=previous+direction,reached=direction>0?next>=to:next<=to,position=reached?to:next,nextState:DemoToken["state"]=position<=0?"yard":position>=57?"finished":position>=52?"home":"track";const nextTokens=displayRef.current.map(t=>keyOf(t)===key?{...t,position,state:nextState}:t);displayRef.current=nextTokens;setDisplayTokens(nextTokens);if(position>=57)emitAudio("finish");else if(nextState==="home")emitAudio("home");else emitAudio("move");if(reached){delete timersRef.current[key];return;}timersRef.current[key]=window.setTimeout(advance,220)};
 ...
 const legal=new Set(legalTokenKeys),palette=BOARD_PALETTES[theme]||BOARD_PALETTES.classic;
 const boardTokens=useMemo(()=>displayTokens.filter(t=>!launchingKeys.has(keyOf(t))&&!(t.state==="track"&&t.position>=1&&t.position<=51)),[displayTokens,launchingKeys]);
 ...
 return <div className="mp-board-wrap" style={{position:"relative",width:"100%",aspectRatio:"1",...style}}>
  ...
  <div className="mp-overlay" aria-hidden="true">
   {trackTokens.map(token=>{const pos=cellPosition(token);if(!pos)return null;return <button key={`token-${keyOf(token)}`} type="button" aria-label={`${token.color} token ${token.id+1}`} onClick={()=>onTokenClick?.(token.color,token.id)} style={{position:"absolute",left:pos[0],top:pos[1],width:"5.1%",aspectRatio:1,borderRadius:"50%",border:"2px solid #222",background:palette[token.color],transform:"translate(-50%,-50%)",zIndex:30,padding:0,cursor:legal.has(keyOf(token))?"pointer":"default",pointerEvents:onTokenClick?"auto":"none"}}/>})}
   {launchTokens.map(token=>{const yard=YARD_CENTERS[token.color]?.[token.id];const start=cellPosition({...token,state:"track",position:1});if(!yard||!start)return null;const yardX=yard[1],yardY=yard[0],startX=Number.parseFloat(start[0]),startY=Number.parseFloat(start[1]);return <span key={`launch-${keyOf(token)}`} style={{position:"absolute",left:`${yardX}%`,top:`${yardY}%`,width:"5.1%",aspectRatio:1,borderRadius:"50%",border:"2px solid #222",background:palette[token.color],transform:"translate(-50%,-50%)",zIndex:30,pointerEvents:"none",animation:"mpLaunch 220ms cubic-bezier(.22,.8,.32,1) forwards",["--launch-dx" as string]:`calc(${startX}% - ${yardX}%)`,["--launch-dy" as string]:`calc(${startY}% - ${yardY}%)`} as React.CSSProperties}/>})}
   {displayTokens.map(token=>{if(!legal.has(keyOf(token))||launchingKeys.has(keyOf(token)))return null;const pos=cellPosition(token);if(!pos)return null;const yard=token.state==="yard";return <span key={`pulse-${keyOf(token)}`} aria-hidden="true" style={{position:"absolute",left:pos[0],top:pos[1],width:yard?"5.1%":"5.9%",aspectRatio:1,transform:"translate(-50%,-50%)",borderRadius:"50%",border:`1.5px solid ${palette[token.color]}`,background:"transparent",boxShadow:`0 0 0 1px rgba(255,255,255,.75),0 0 5px ${palette[token.color]}`,animation:"mpLegalBreath 1.25s ease-in-out infinite",pointerEvents:"none",zIndex:31}}/>})}
  </div>
  <style jsx global>{`@keyframes mpLegalBreath{0%,100%{opacity:.35;transform:translate(-50%,-50%) scale(.97)}50%{opacity:1;transform:translate(-50%,-50%) scale(1.025)}}@keyframes mpLaunch{0%{transform:translate(-50%,-50%)}100%{transform:translate(-50%,-50%) translate(var(--launch-dx),var(--launch-dy))}}.mp-board-wrap{position:relative}.mp-overlay{position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:40}`}</style>
 </div>;
}
