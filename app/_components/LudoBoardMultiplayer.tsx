...
function toRenderToken(t: DemoToken): DemoToken { if (t.state === "track" && t.position >= 1 && t.position <= 51) return { ...t, position: t.position - 1, state: "track" }; return t; }
type OverlayPosition = [string, string];
function cellPosition(t: DemoToken): OverlayPosition | null {
  if (t.state === "yard") return YARD_CENTERS[t.color]?.[t.id] || YARD_CENTERS[t.color]?.[0] || null;
