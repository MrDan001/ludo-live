...
function cellPosition(t: DemoToken): [string, string] | null {
  ...
  if (t.state === "track" && t.position >= 1 && t.position <= 51) {
    const cell = getRenderTokenCell(t.color, t.position);
...
export default function LudoBoardMultiplayer({ theme="classic", preview=false, className="", style, demoTokens=[], onTokenClick, snapOnUpdate=false, finishSound=false, animateUpdates=true, legalTokenKeys=[] }: Props) {
  ...
  useEffect(() => {
    ...
    if (snapOnUpdate || !animateUpdates) {
      Object.values(timersRef.current).forEach(timer => window.clearTimeout(timer)); Object.values(launchTimersRef.current).forEach(timer => window.clearTimeout(timer));
      timersRef.current = {}; launchTimersRef.current = {}; setLaunchingKeys(new Set()); displayRef.current = Array.from(incoming.values()); setDisplayTokens(displayRef.current); return;
    }
    ...
    for (const [key, target] of incoming) {
      ...
      if (from === 0 && to === 1 && target.state === "track") {
        ...
        displayRef.current = launched; setDisplayTokens(launched); setLaunchingKeys(prev => new Set(prev).add(key));
        if (launchTimersRef.current[key]) window.clearTimeout(launchTimersRef.current[key]);
        launchTimersRef.current[key] = window.setTimeout(() => { setLaunchingKeys(prev => { const next = new Set(prev); next.delete(key); return next; }); delete launchTimersRef.current[key]; }, 220);
        emitAudio("move"); delete timersRef.current[key]; continue;
      }
  ...
  useEffect(() => () => { Object.values(timersRef.current).forEach(timer => window.clearTimeout(timer)); Object.values(launchTimersRef.current).forEach(timer => window.clearTimeout(timer)); }, []);
  const legal = new Set(legalTokenKeys); const palette = BOARD_PALETTES[theme] || BOARD_PALETTES.classic;
  const boardTokens = useMemo(() => displayTokens.filter(t => !launchingKeys.has(keyOf(t))), [displayTokens, launchingKeys]);
