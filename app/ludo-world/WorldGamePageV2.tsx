...
export default function WorldGamePageV2({ mode }: { mode: WorldMode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (customization?.equippedBoard) setBoardTheme(String(customization.equippedBoard) as BoardThemeId);
      } finally {
  ...
  const finishRound = (winner: WorldTurn) => {
    ...
    setSeriesScore((score) => {
      ...
      if (next[winner] >= (engine.seriesToWin ?? 2)) schedule(() => finishMatch(winner), 20);
      else {
        clearAllTimers();
        setDice(null);
        setBotRolling(false);
  ...
  useEffect(() => {
    if (turn !== "bot" || gameOver || finishedRef.current) return;
    const before = tokens;
    ...
    void run();
    return () => { /* timers are centrally cleaned when turn changes or reset occurs */ };
  ...
  const substatus = mode === "tournament" ? `SERIES ${seriesScore.human} : ${seriesScore.bot} · ROUND ${round}` : mode === "speed" && turn === "human" ? `${seconds}s remaining` : message;
  ...
  return <div className="lw2-page">
    ...
    <section className={`lw2-dice-card ${turn === "human" ? "human-ready" : "bot-active"}`}>
      <div className="lw2-dice"><DemoDice value={dice ?? 1} onRoll={handleHumanRoll} disabled={turn !== "human" || dice != null || gameOver} botRolling={botRolling} skin={diceSkin as any} /></div>
