...
const BOT_DICE_DISPLAY_MS = 900;
const BOT_STEP_MS = 300;
const BOT_SETTLE_MS = 500;
const BOT_EXTRA_TURN_GAP_MS = 650;
...
export default function WorldGamePageMobileV6({ mode }: { mode: WorldMode }) {
  ...
  const humanTokens = useMemo(() => tokens.filter((t) => HUMAN_COLORS.includes(t.color as any)), [tokens]);
  const legalTokenKeys = useMemo(() => {
    if (turn !== "human" || dice == null || gameOver) return [];
    return humanTokens.filter((t) => canMove(tokens, t as any, dice as any)).map((t) => `${t.color}:${t.id}`);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      while (keepBotTurn && mountedRef.current && !cancelled && !finishingRef.current) {
        ...
        await waitBot(BOT_DICE_DISPLAY_MS);
        if (cancelled || !mountedRef.current || finishingRef.current) break;

        // Keep the exact rolled value visible while the board counts the move.
        setBotRolling(false);
        ...
        if (!legal.length) {
          ...
          setDice(null); keepBotTurn = decision.nextTurn === "bot"; setMessage(decision.message); if (!keepBotTurn) setTurn("human"); else await waitBot(BOT_EXTRA_TURN_GAP_MS);
          continue;
        }
        ...
        if (yardLaunch) {
          ...
          currentTokens = staged;
          tokensRef.current = staged;
          setTokens(staged);
          await waitBot(BOT_STEP_MS);
        } else {
          ...
          // Re-apply the canonical final state so captures and final-state flags are authoritative.
          currentTokens = fullMove.next;
        ...
        const decision = engine.resolveTurn({
          side: "bot", dice: rolled, captured: fullMove.captured, before: beforeMoveTokens, after: currentTokens,
          simulate: (candidate) => { const r = applyMove(beforeMoveTokens, candidate, rolled); return r ? { token: candidate, dice: rolled, tokens: r.next, captured: r.captured } : null; },
        });
        setDice(null);
        setMessage(decision.message);
        keepBotTurn = decision.nextTurn === "bot";
        if (!keepBotTurn) setTurn("human");
        else await waitBot(BOT_EXTRA_TURN_GAP_MS);
      }
      setBotRolling(false);
