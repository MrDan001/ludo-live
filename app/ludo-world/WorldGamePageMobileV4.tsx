...
function validSavedState(value: unknown): value is SavedState {
  ...
  return s.version === 4
    ...
    && (s.turn === "human" || s.turn === "bot")
    && (s.dice === null || s.dice === 1 || s.dice === 2 || s.dice === 3 || s.dice === 4 || s.dice === 5 || s.dice === 6)
    && typeof s.gameOver === "boolean"
    && typeof s.message === "string"
    && !!s.seriesScore
    && typeof s.seriesScore.human === "number"
    && typeof s.seriesScore.bot === "number"
    && typeof s.round === "number";
}

export default function WorldGamePageMobileV4({ mode }: { mode: WorldMode }) {
  const engine: WorldEngine = WORLD_ENGINES[mode];
  const storageKey = `ludo-world-match:${mode}:v4`;
  ...
  useEffect(() => {
    if (!ready) return;
    try {
      const snapshot: SavedState = { version: 4, tokens, turn, dice, gameOver, message, seriesScore, round };
  ...
  const waitBot = (ms: number) => new Promise<void>((resolve) => {
    const id = window.setTimeout(() => {
      botTimersRef.current = botTimersRef.current.filter((item) => item !== id);
      resolve();
    }, ms);
  ...
  const finishRound = (winner: WorldTurn) => {
    ...
    setSeriesScore((score) => ({ ...score, [winner]: score[winner] + 1 }));
    const nextHuman = winner === "human" ? seriesScore.human + 1 : seriesScore.human;
    const nextBot = winner === "bot" ? seriesScore.bot + 1 : seriesScore.bot;
    if (Math.max(nextHuman, nextBot) >= (engine.seriesToWin ?? 2)) {
      finishMatch(winner);
      return;
    }
    stopSpeedTimer();
  ...
  const handleHumanRoll = (rolled: DiceValue) => {
    ...
    if (!legal.length) {
      ...
      const id = window.setTimeout(() => resolveTurn("human", rolled, snapshot, snapshot, false, true), 600);
      botTimersRef.current.push(id);
      return;
    }
  ...
  useEffect(() => {
    if (!ready || !engine.humanTurnSeconds || turn !== "human" || dice !== null || gameOver || finishingRef.current) {
      stopSpeedTimer();
      return;
    }
    setSeconds((value) => value > 0 ? value : engine.humanTurnSeconds ?? 0);
    stopSpeedTimer();
    speedTimerRef.current = window.setInterval(() => {
      setSeconds((current) => {
        if (current <= 1) {
          ...
          const id = window.setTimeout(() => setTurn("bot"), 180);
          botTimersRef.current.push(id);
          return 0;
        }
        return current - 1;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      while (keepBotTurn && mountedRef.current && !cancelled && !gameOver && !finishingRef.current) {
        setBotRolling(true);
        ...
        if (!legal.length) {
          const decision = engine.resolveTurn({
            ...
            after: currentTokens,
            simulate: (candidate) => {
              const result = applyMove(currentTokens, candidate, rolled);
              return result ? { token: candidate, dice: rolled, tokens: result.next, captured: result.captured } : null;
            },
          });
          setDice(null);
          setBotRolling(false);
          setMessage(decision.message);
          keepBotTurn = decision.nextTurn === "bot";
          if (!keepBotTurn) setTurn("human");
          continue;
        }
        ...
        await waitBot(260);
        if (cancelled || !mountedRef.current || finishingRef.current) break;
        const current = currentTokens.find((token) => token.color === choice.color && token.id === choice.id);
        const result = current ? applyMove(currentTokens, current, rolled) : null;
        if (!result) {
          keepBotTurn = false;
          setTurn("human");
          setDice(null);
          setBotRolling(false);
          setMessage("Opponent could not complete its move. Your turn.");
          break;
        }

        currentTokens = result.next;
        tokensRef.current = currentTokens;
        setTokens(currentTokens);

        if (hasWon(currentTokens as any, BOT_COLORS as any)) {
          keepBotTurn = false;
          finishRound("bot");
          break;
        }
        ...
        const decision = engine.resolveTurn({
          ...
          before: tokensRef.current,
          after: currentTokens,
          simulate: (candidate) => {
            const sim = applyMove(currentTokens, candidate, rolled);
      ...
      setDice((value) => value === null ? null : value);
      setBotRolling(false);
      botRunningRef.current = false;
    };
  ...
  return (
    <main className="lw3-page">
      ...
      <section className="lw3-players">
        <div className="lw3-player lw3-player-you"><div className="lw3-avatar"><EquippedAvatar style={{ width: 54, height: 54 }} /></div><div><small>YOU</small><b>{user.username}</b><span>LEVEL {user.level}</span></div></div>
        <div className="lw3-vs">VS</div>
        <div className="lw3-player lw3-player-opponent"><div className="lw3-opponent-icon">{ICONS[mode]}</div><div><small>OPPONENT</small><b>{opponent}</b><span>{mode === "boss" ? "ELITE AI" : mode === "teams" ? "AI TEAM" : "WORLD AI"}</span></div></div>
      </section>
      ...
      <section className="lw3-dice-card">
        <div className="lw3-dice-head"><div><small>{botRolling ? "WORLD AI" : turn === "human" ? "YOUR DICE" : "WORLD AI"}</small><b>{gameOver ? "Match complete" : botRolling ? "Rolling…" : dice != null ? `Rolled ${dice}` : "Tap to roll"}</b></div><span className="lw3-skin">{diceSkin.toUpperCase()}</span></div>
        <div className="lw3-dice-stage"><DemoDice value={dice ?? 1} onRoll={handleHumanRoll} disabled={turn !== "human" || dice !== null || gameOver || botRolling} botRolling={botRolling} skin={diceSkin} /></div>
