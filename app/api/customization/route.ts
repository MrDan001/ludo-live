...
export async function POST(q: NextRequest) {
  ...
  try {
    ...
    const row = await client.query<any>(
      `SELECT coins,gems,owned_boards,owned_dice,equipped_board,equipped_dice,
              owned_avatars,equipped_avatar,owned_items,equipped_items
       FROM ludo_users WHERE id=$1 FOR UPDATE`,
      [u.id]
    );
