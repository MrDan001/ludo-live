...
export async function GET(q: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      ownedBoards: u ? clean(u.owned_boards || ["classic"]) : ["classic"],
      ownedDice: u ? clean(u.owned_dice || ["classic"]) : ["classic"],
      equippedBoard: u?.equipped_board || "classic",
      equippedDice: u?.equipped_dice || "classic",
      ownedAvatars: u ? clean(u.owned_avatars) : [],
      equippedAvatar: u?.equipped_avatar || "default",
      ownedItems: u ? clean(u.owned_items) : [],
      equippedItems: u ? clean(u.equipped_items) : [],
