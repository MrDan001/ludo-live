...
async function ensureWalletAudit() {
  await pool.query(`
    ...
    BEGIN
      FOREACH c IN ARRAY ARRAY['coins','gems'] LOOP
        ...
        IF old_balance <> new_balance THEN
          ...
          why := NULLIF(current_setting('ludo.wallet_reason',true),'');
...
export async function POST(q: NextRequest) {
  try {
    ...
    if (!uid || !currency || !Number.isSafeInteger(amount) || amount === 0) return NextResponse.json({ error: "Player, currency and a non-zero amount are required." }, { status: 400 });
    ...
    try {
      ...
      const before = Number(r.rows[0][currency]);
