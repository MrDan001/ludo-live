...
import { DEFAULT_SPIN_WHEEL, SPIN_SLOT_COUNT, cleanSpinSlot, isSpinKind } from "../../../../lib/spinWheel";
...
async function getAdmin(request: NextRequest) {
  ...
  const allowed = (process.env.ADMIN_EMAILS || process.env.ADMIN_EMAIL || "").split(",").map((value) => value.trim().toLowerCase()).filter(Boolean);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.query(`SELECT slot,id,kind,label,icon,amount,probability,item_type AS "itemType",item_id AS "itemId",updated_at AS "updatedAt" FROM ludo_spin_wheel_slots ORDER BY slot ASC`);
    const rewards = result.rows.map((row: any) => ({ ...row, amount: Number(row.amount), probability: Number(row.probability) }));
    return NextResponse.json({ slotCount: SPIN_SLOT_COUNT, rewards, catalog: catalog.filter((item: any) => ["board", "dice", "avatar", "item"].includes(item.type)) }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error(error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!Number.isInteger(slotNumber) || slotNumber < 0 || slotNumber >= SPIN_SLOT_COUNT) return NextResponse.json({ error: "Spin slot must be between 1 and 8." }, { status: 400 });
    ...
    if ((reward.kind === "coins" || reward.kind === "gems" || reward.kind === "extraSpin") && reward.amount < 1) return NextResponse.json({ error: "Amount must be at least 1 for this reward type." }, { status: 400 });
    if (reward.kind === "shop_item" && (!reward.itemType || !reward.itemId)) return NextResponse.json({ error: "Select a Shop item for this slot." }, { status: 400 });
    ...
    transaction = true;
    await client.query(`UPDATE ludo_spin_wheel_slots SET id=$1,kind=$2,label=$3,icon=$4,amount=$5,probability=$6,item_type=$7,item_id=$8,updated_at=NOW() WHERE slot=$9`, [reward.id, reward.kind, reward.label, reward.icon, reward.amount, reward.probability, reward.itemType, reward.itemId, reward.slot]);
    await client.query(`INSERT INTO ludo_admin_actions(admin_user_id,action,target_user_id,details) VALUES($1,'spin_wheel_slot_update',NULL,$2)`, [user.id, JSON.stringify({ slot: reward.slot + 1, reward })]);
    await client.query("COMMIT");
    transaction = false;
    return NextResponse.json({ ok: true, slot: reward.slot, reward }, { headers: { "Cache-Control": "no-store" } });
  } catch (error: any) {
