...
async function setup() {
  ...
  await pool.query(`ALTER TABLE ludo_mission_definitions ADD COLUMN IF NOT EXISTS scheduled_at TIMESTAMPTZ`);
}
...
export async function GET(q: NextRequest) {
  try {
    ...
    await setup();
    const r=await pool.query(`SELECT id,title,description,target,reward_coins AS "rewardCoins",reward_gems AS "rewardGems",kind,admin_created AS "adminCreated",active,scheduled_date AS "scheduledDate",scheduled_at AS "scheduledAt",created_at AS "createdAt" FROM ludo_mission_definitions ORDER BY admin_created DESC,COALESCE(scheduled_at,created_at) DESC`);
    return NextResponse.json({missions:r.rows});
...
export async function POST(q: NextRequest) {
  try {
    const a = await admin(q); if (!a) return NextResponse.json({error:"Admin access required."},{status:403});
    await setup();
    const b=await q.json(); const action=String(b.action||"");
    if(action==="create"||action==="edit"){
      const id=String(b.id||`admin-mission-${Date.now()}`).toLowerCase().replace(/[^a-z0-9-]/g,"-").slice(0,80);
      const title=String(b.title||"").trim().slice(0,120);
      const description=String(b.description||"").trim().slice(0,300);
      const target=Math.max(1,Math.min(100000,Math.trunc(Number(b.target)||1)));
      ...
      const kind=String(b.kind||"play_games");
      ...
      if(scheduledAt&&Number.isNaN(scheduledAt.getTime())) return NextResponse.json({error:"Invalid mission date/time."},{status:400});
      const scheduledIso=scheduledAt?scheduledAt.toISOString():null;
      const scheduledDate=scheduledAt?scheduledAt.toISOString().slice(0,10):null;
      if(action==="create") await pool.query(`INSERT INTO ludo_mission_definitions(id,title,description,target,reward_coins,reward_gems,kind,admin_created,active,scheduled_date,scheduled_at) VALUES($1,$2,$3,$4,$5,$6,$7,TRUE,$8,$9,$10,$11)`,[id,title,description,target,rewardCoins,rewardGems,kind,active,scheduledDate,scheduledIso]);
      else { const r=await pool.query(`UPDATE ludo_mission_definitions SET title=$1,description=$2,target=$3,reward_coins=$4,reward_gems=$5,kind=$6,active=$7,scheduled_date=$8,scheduled_at=$9 WHERE id=$10 AND admin_created=TRUE`,[title,description,target,rewardCoins,rewardGems,kind,active,scheduledDate,scheduledIso,id]); if(!r.rowCount)return NextResponse.json({error:"Admin mission not found."},{status:404}); }
      await pool.query(`INSERT INTO ludo_admin_actions(admin_user_id,action,details) VALUES($1,$2,$3)`,[a.id,action==="create"?"create_mission":"edit_mission",JSON.stringify({id,title,target,rewardCoins,rewardGems,kind,active,scheduledAt:scheduledIso})]); return NextResponse.json({ok:true,id});
    }
