...
const MILESTONE_UNLOCKS: Unlock[] = [
  ...
  { type: "avatar", id: "avatar-6", name: "Legendary Wizard Avatar", icon: "🧙🏽‍♂️", fallbackGems: 100 },
    ...
    try {
      ...
      while (xp >= requiredForLevel(level)) {
        ...
        const inserted = await client.query<any>(
          `INSERT INTO ludo_level_rewards(user_id,level,coins,gems,badge_id,reward_type,reward_id,reward_name,title)
           VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)
           ON CONFLICT(user_id,level) DO NOTHING
           RETURNING coins,gems,badge_id,reward_type,reward_id,reward_name`,
          [user.id, level, reward.coins, reward.gems, reward.badge, reward.unlock?.type || null, reward.unlock?.id || null, reward.unlock?.name || null, `Level ${level} Reward`]
        );
        ...
        if (reward.unlock) {
          ...
          if (!list.includes(reward.unlock.id)) {
            ...
            // If a player already owns the milestone cosmetic, preserve value with a smaller gem fallback.
            rewardGems += reward.unlock.fallbackGems;
          }
        }
      }
