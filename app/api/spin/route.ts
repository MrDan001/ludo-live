...
import { DEFAULT_SPIN_WHEEL, SPIN_SLOT_COUNT, cleanSpinSlot, weightedPick, type SpinKind, type SpinWheelSlot } from "../../../lib/spinWheel";
...
async function ensureSchema() {
  ...
  await pool.query(`DELETE FROM ludo_spin_wheel_slots WHERE slot NOT BETWEEN 0 AND 7`);
}

async function getWheel(db = pool): Promise<{ wheel: DbSlot[]; version: string }> {
  const result = await db.query<DbSlot>(
...
function publicPayload(extra: Record<string, unknown> = {}) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      publicPayload({ spins: Number(row?.spins || 0), totalSpins: Number(row?.total_spins || 0), wheel, wheelVersion: version }),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const currentTotal = Number(state.rows[0]?.total_spins || 0);
    if (currentSpins < 1) {
    ...
    if (prize.kind === "coins" || prize.kind === "gems") {
      ...
      await adjustWallet(client, user.id, prize.kind, prize.amount, {
        source: "spin_wheel",
        sourceRef: prize.id,
      ...
      await client.query(
        ...
        [user.id, prize.itemType, prize.itemId, prize.label || item.name, prize.icon || item.icon || "✨"],
      );
    }
