...
async function ensureActivitySchema() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS ludo_level_rewards(
...
function requiredForLevel(level: number) {
  return 10 + Math.max(1, Math.floor(Number(level) || 1)) * 5;
}
  ...
  try {
    ...
    try {
      ...
      const stateResult = await client.query<{
        ...
        "SELECT spins,active_seconds,total_active_seconds,last_heartbeat_at FROM ludo_spin_state WHERE user_id=$1 FOR UPDATE",
        [id]
      );
      ...
      if (activeSeconds >= INTERVAL_SECONDS) {
        ...
        granted = intervals * spinsPerInterval;
        xpGranted = intervals * xpPerInterval;
        spins += granted;
      ...
      if (xpGranted > 0) {
        const userResult = await client.query<any>(
          "SELECT xp,level,coins,gems,owned_boards,owned_dice,owned_avatars,owned_items FROM ludo_users WHERE id=$1 FOR UPDATE",
          [id]
        );
        ...
        const owned = {
          board: Array.isArray(user.owned_boards) ? user.owned_boards.map(String) : ["classic"],
          dice: Array.isArray(user.owned_dice) ? user.owned_dice.map(String) : ["classic"],
          avatar: Array.isArray(user.owned_avatars) ? user.owned_avatars.map(String) : [],
          item: Array.isArray(user.owned_items) ? user.owned_items.map(String) : [],
        };
        ...
        await client.query(
          `UPDATE ludo_users
           ...
           WHERE id=$9`,
          [xp, level, rewardCoins, rewardGems, JSON.stringify(owned.board), JSON.stringify(owned.dice), JSON.stringify(owned.avatar), JSON.stringify(owned.item), id]
        );
      } else {
        const userResult = await client.query<{ xp:number; level:number }>("SELECT xp,level FROM ludo_users WHERE id=$1", [id]);
        xp = Number(userResult.rows[0]?.xp || 0);
        level = Math.max(1, Number(userResult.rows[0]?.level) || 1);
      }

      await client.query(
        `INSERT INTO ludo_spin_state(user_id,spins,total_spins,active_seconds,total_active_seconds,last_heartbeat_at)
         VALUES($1,$2,0,$3,$4,$5)
         ON CONFLICT(user_id) DO UPDATE SET spins=$2,active_seconds=$3,total_active_seconds=$4,last_heartbeat_at=$5`,
        [id, spins, activeSeconds, totalActiveSeconds, now]
      );
