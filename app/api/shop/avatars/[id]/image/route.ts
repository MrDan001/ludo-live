...
export async function GET(_q: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return new NextResponse(row.image_data, { status: 200, headers: { "Content-Type": row.image_type || "image/webp", "Cache-Control": "public, max-age=31536000, immutable", "X-Content-Type-Options": "nosniff" } });
  } catch (e) {
    console.error(e);
