...
export default function MultiplayerGame(){
 ...
 const [notice,setNotice]=useState("Connecting to the match…");
 ...
 useEffect(()=>{
  ...
  (async()=>{
   let pid="";
   try{const a=await fetch("/api/auth",{cache:"no-store"}).then(x=>x.json());pid=String(a?.user?.id||"")}catch{}
   if(!alive)return;
   setMe(pid);
   const s=io(location.origin,{transports:["websocket","polling"]});
   setSocket(s);
   s.on("connect",()=>setNotice("Connected. Waiting for the match state…"));
   s.on("game-state",(g:GameState)=>{if(!alive)return;setGame(g);applyServerTokens(g.tokens||{});setPending(g.currentPlayerId===pid?g.pendingMove:null);setRoll(g.dice||1);setNotice(g.currentPlayerId===pid?(g.pendingMove?`You rolled ${g.pendingMove}. Select a legal token to move.`:"Your turn — roll the dice."):`${g.players.find(p=>p.playerId===g.currentPlayerId)?.name||"Player"}'s turn.`)});
   s.on("game-dice",({playerId,value}:{playerId:string;value:Face})=>{setRoll(value);if(playerId===pid){setPending(value);setNotice(`You rolled ${value}. Select a legal token to move.`)}else{setRemoteRolling(true);setNotice("Opponent rolled the dice.");window.setTimeout(()=>setRemoteRolling(false),950)}});
   ...
   s.on("disconnect",()=>setNotice("Connection lost. Reconnecting…"));
   return()=>{alive=false;s.disconnect()};
  })();
  return()=>{alive=false};
 },[]);
 useEffect(()=>{if(!socket||!game||!myTurn||pending==null)return;if(hasLegalMove(pending))return;const r=pending;setPending(null);socket.emit("game-move",{tokenId:"__skip__",to:0});setNotice(r===6?"No legal move. You get the bonus roll.":"No legal move. Turn passes to the next player.")},[socket,game,myTurn,pending,tokens,myColors]);
 const chooseToken=(color:Color,id:number)=>{if(!socket||!game||!myTurn||pending==null||animating||!myColors.includes(color))return;const t=tokens.find(x=>x.color===color&&x.id===id);if(!t||!legalTarget(t.position,pending))return;const target=targetFor(t.position,pending);setPending(null);socket.emit("game-move",{tokenId:`${color}:${id}`,to:target});setNotice("Move sent. Waiting for the match to update…")};
 const handleRoll=()=>{if(!socket||!game||!myTurn||pending!==null||animating)return;socket.emit("game-roll")};
 useEffect(()=>{try{const saved=localStorage.getItem("ludo-match-board");if(saved&&saved in BOARD_PALETTES)setTheme(saved as BoardThemeId)}catch{}},[]);
 const p=BOARD_PALETTES[theme]||BOARD_PALETTES.classic;
 return (
  <AppFrame back="/lobby">
   <main style={{...page,background:p.bg} as React.CSSProperties}>
    <header style={top}><div style={eyebrow}>LIVE MATCH</div></header>
    ...
    <section style={controls}>
     <div style={controlCopy}>
      <div style={turnLabel}>{myTurn?"YOUR TURN":currentId?`${players.find(pl=>pl.playerId===currentId)?.name||"PLAYER"} TURN`:"MATCH"}</div>
      <b style={headline}>{pending!==null?"Move your token":"Roll the dice"}</b>
      <p style={sub2}>{notice}</p>
     </div>
...
const page:React.CSSProperties={width:"100%",minHeight:"calc(100vh - 40px)",padding:"18px 14px 28px",boxSizing:"border-box",borderRadius:22};
const top:React.CSSProperties={display:"flex",alignItems:"center",marginBottom:4,paddingLeft:2};
const eyebrow:React.CSSProperties={fontSize:15,letterSpacing:3,color:"#182033",fontWeight:950};
const sub2:React.CSSProperties={color:"#a9c0e5",margin:"6px 0 0",fontSize:15,lineHeight:1.35};
const boardWrap:React.CSSProperties={display:"grid",placeItems:"center",marginTop:4};
const boardShell:React.CSSProperties={width:"100%",maxWidth:620,position:"relative"};
const controls:React.CSSProperties={display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginTop:16,padding:"18px 16px",borderRadius:22,background:"#071a36",border:"1px solid #284b7b",boxShadow:"0 10px 30px rgba(7,26,54,.18)"};
const controlCopy:React.CSSProperties={minWidth:0,flex:1};
const turnLabel:React.CSSProperties={fontSize:11,letterSpacing:2,color:"#60a5fa",fontWeight:900,textTransform:"uppercase"};
