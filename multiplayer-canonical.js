...
if (!Socket.prototype.__ludoOnlineAuthorityV4) {
  ...
  const broadcastConnection = (room, pid, connected, reason) => { room?.nsp?.to(room.code).emit("game-connection", { roomCode: room.code, playerId: String(pid), connected: !!connected, reason: reason || null }); };
  const forfeitPlayer = async (socket, room, pid, reason = "opponent_left") => {
    ...
    if (remaining.length === 1) {
      ...
      room.game.status = "finished"; room.game.winnerId = String(winner.playerId); room.game.currentPlayerId = null;
    ...
    } else { room.nsp?.to(room.code).emit("game-player-left", { playerId: String(pid), remaining: remaining.map((m) => String(m.playerId)) }); emitState(room); }
    return true;
  };
  const scheduleForfeit = (room, pid) => {
    ...
    member.reconnectTimer = setTimeout(async () => {
      ...
      const liveRoom = games.get(room.code); const current = liveRoom?.members.get(pid);
  ...
  Socket.prototype.on = function (event, listener) {
    if (event === "join-room") return originalOn.call(this, event, function (payload = {}, ...extra) {
      const next = { ...payload }; const code = String(next.roomCode || "").trim().toUpperCase(); const pid = String(this.data?.authUserId || next.playerId || "").trim();
    ...
    if (event === "start-game") return originalOn.call(this, event, function (...args) {
      const ack = ackFrom(args); const code = roomCode(this), pid = playerId(this), room = games.get(code);
