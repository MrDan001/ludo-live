...
// Keep completed/inactive rooms out of every lobby room-list response.
// This only changes room-list visibility; it does not change room/game rules.
const activeRoomList = ')).filter(r=>r.players<r.roomSize)';
const filteredActiveRoomList = ')).filter(r=>!rooms.get(r.code)?.game&&r.players>0&&r.players<r.roomSize)';
if (source.includes(activeRoomList) && !source.includes(filteredActiveRoomList)) {
  source = source.replace(activeRoomList, filteredActiveRoomList);
}
...
const hasActiveRoomListFilter = source.includes(filteredActiveRoomList);
let positions = handlerPositions();
if (hasCanonicalLifecycle && hasActiveRoomListFilter && positions.length === 1) {
