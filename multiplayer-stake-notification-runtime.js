...
globalThis.__ludoCreateWinnerNotification=async(roomCode,winnerId,winnerName)=>{
  const code=String(roomCode||"").trim().toUpperCase();const uid=String(winnerId||"").trim();
  if(!code||!uid)return;
  await ensure();
  const client=await db().connect();
  try{
    await client.query("BEGIN");
    const bet=await client.query("SELECT id,pot,stake_per_player FROM ludo_multiplayer_match_bets WHERE room_code=$1 LIMIT 1",[code]);
    if(!bet.rowCount){await client.query("COMMIT");return;}
    const row=bet.rows[0];
    const title="🏆 Multiplayer Victory!";
    const message=`Congratulations ${String(winnerName||"Player")}! You were declared the winner because the opponent left the game. ${Number(row.pot)||0} coins have been awarded to your profile.`;
...
ensure().catch(e=>console.error("[multiplayer-stake-notification] schema initialization failed",e));
