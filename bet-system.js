...
async function lockBet(room) {
  ...
  try {
    ...
    if (existing.rowCount && (existing.rows[0].status === 'locked' || existing.rows[0].status === 'settled')) {
      await client.query('COMMIT'); room.locked=existing.rows[0].status==='locked'; room.betStatus=existing.rows[0].status; room.betId=String(existing.rows[0].id); return {ok:true};
    }
...
async function settleBet(room,winnerId) {
  ...
  try {
    ...
    try { await client.query('ROLLBACK'); } catch {}
    console.error('[bet] settlement failed',room.code,e); return {ok:false,error:e instanceof Error?e.message:'Unable to settle the bet.'};
...
function installBetSystem() {
  ...
  Socket.prototype.on=function(event,listener) {
    if (event==='join-room') return originalOn.call(this,event,async(...args)=>{
      ...
      if (!room) {
        ...
        room={code,roomSize:size,stake:validStake(requested)?requested:MIN_STAKE,members:new Map(),locked:false,betStatus:'open',betId:null}; rooms.set(code,room);
  ...
  // Canonical settlement hook: the existing multiplayer server emits
  // game-state { status:'finished', winnerId } after authoritative Ludo rules.
  const originalTo=Server.prototype.to;
  Server.prototype.to=function(...args) {
    const server=this;
    const op=originalTo.apply(server,args);
    ...
    op.emit=function(event,...payload) {
      ...
      if(event==='roster'&&Array.isArray(payload[0])&&room) {
        ...
        for(const m of payload[0]) {
          const socketId=String(m.id||m.socketId||''); const playerId=String(m.playerId||m.id||'');
      ...
      if(event==='game-state'&&room&&room.betStatus==='locked') {
        ...
        if(game.status==='finished'&&winnerId) {
          void settleBet(room,winnerId).then(settled=>{
            if(settled.ok&&!settled.duplicate) op.emit('bet-settled',{winnerId,pot:settled.pot,stake:room.stake,roomSize:room.roomSize});
            broadcastState(server,room);
          });
        }
      }
