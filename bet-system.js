...
function state(room) {
  return { enabled: true, stake: room.stake, minStake: MIN_STAKE, maxStake: MAX_STAKE, pot: room.stake * room.roomSize, roomSize: room.roomSize, locked: !!room.locked, status: room.betStatus || 'open' };
}
...
async function lockBet(room) {
  ...
  try {
    ...
    if (existing.rowCount && (existing.rows[0].status === 'locked' || existing.rows[0].status === 'settled')) {
      await client.query('COMMIT'); room.locked = true; room.betStatus = existing.rows[0].status; room.betId = String(existing.rows[0].id); return { ok: true };
    }
    ...
    for (const id of ids) {
      ...
      await client.query('INSERT INTO ludo_match_bet_players(bet_id,user_id,stake) VALUES($1,$2,$3)', [betId, id, room.stake]);
    }
    await client.query('COMMIT');
    room.locked = true; room.betStatus = 'locked'; room.betId = String(betId);
    return { ok: true };
  } catch (e) {
...
async function settleBet(room, winnerId) {
  ...
  try {
    ...
    await client.query('COMMIT');
    room.betStatus = 'settled'; room.locked = false; room.settledWinnerId = winnerId;
    return { ok: true, pot };
  } catch (e) {
    try { await client.query('ROLLBACK'); } catch {}
...
function installBetSystem() {
  ...
  Socket.prototype.on = function(event, listener) {
    if (event === 'join-room') {
      return originalOn.call(this, event, async (...args) => {
        ...
        if (!room) {
          ...
          room = { code, roomSize: size, stake: validStake(requested) ? requested : MIN_STAKE, members: new Map(), locked: false, betStatus: 'open', moves: new Map() };
          rooms.set(code, room);
    ...
    if (event === 'disconnect') {
      return originalOn.call(this, event, (...args) => {
        const room = roomFor(this); if (room) room.members.delete(this.id);
        return listener.apply(this, args);
      });
    }
    if (event === 'start-game') {
    ...
    if (event === 'game-move') {
      return originalOn.call(this, event, async (...args) => {
        ...
        if (!room || room.betStatus !== 'locked' || !pid) return result;
        const token = String(payload.tokenId ?? '');
        const to = Number(payload.to);
        if (!token || !Number.isFinite(to)) return result;
        let player = room.moves.get(pid); if (!player) { player = new Map(); room.moves.set(pid, player); }
        player.set(token, to);
        const winner = [...player.values()].filter(position => position >= 56).length >= 4;
        if (winner) {
          const settled = await settleBet(room, pid);
          if (settled.ok) this.nsp.to(room.code).emit('bet-settled', { winnerId: pid, pot: settled.pot, stake: room.stake });
        }
        return result;
      });
    }
  ...
  Server.prototype.emit = function(event, ...payload) {
    if (event === 'room-list' && Array.isArray(payload[0])) payload[0] = payload[0].map(room => ({ ...room, stake: rooms.get(String(room.code || '').toUpperCase())?.stake || MIN_STAKE }));
    return originalServerEmit.call(this, event, ...payload);
  };
  ...
  Server.prototype.to = function(...args) {
    ...
    op.emit = function(event, ...payload) {
      if (event === 'roster' && Array.isArray(payload[0])) {
        ...
        if (room) {
          ...
          for (const m of payload[0]) {
            ...
            if (socketId) next.set(socketId, { socketId, playerId, name: String(m.name || 'Player'), host: !!m.host, ready: !!m.ready });
          }
          room.members = next;
        }
      }
