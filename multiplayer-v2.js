...
function safe(c,p){const x=cell(c,p);return SAFE_CELLS.some(same.bind(null,x))}
...
function snapshot(g){return {status:g.status,currentPlayerId:g.currentPlayerId,dice:g.dice,pendingMove:g.pendingMove,sixStreak:g.sixStreak,players:g.players.map(p=>({playerId:p.playerId,name:p.name,seat:p.seat,connected:!!p.socket,colors:p.colors,ready:p.ready})),tokens:g.tokens,winnerId:g.winnerId||null,reconnectDeadline:g.reconnectDeadline||null}}
